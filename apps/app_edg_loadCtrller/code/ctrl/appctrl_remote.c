...
void appctrl_remote_init()
{
    ...
    ret = lctrl_read_ctrl(CTRL_REMOTE_TYPE, (uint8*)buf, 4096, &len);
    ...
    for(i = 0; i < OOP_MAX_TURN * 2; i++)
    {
        ...
        {
            set_remote_state_valid(i + 1, g_RemoteInfo.remoteType[i], NULL, NULL);
            set_remote_state_invalid(i + 1);
        }
...
* @Update    :
**********************************************************************/
void appctrl_remote_decress(OOP_DATETIME_S_T *ptTime)
{
    time_t last;
    time_t now;
    int    passtime = 0;
    ...
    DT_DateTimeShort2Time(ptTime, &last);
    time(&now);
    passtime = now - last;
    CTRL_FMT_DEBUG("上电之前时间流逝%ds\n", passtime);
    if(passtime > 0 && last > 0)
    {
        for(i = 0; i < OOP_MAX_TURN * 2; i++)
        {
            if(g_RemoteInfo.wOpenBrakeDelay[i] > passtime)
            {
                g_RemoteInfo.wOpenBrakeDelay[i] -= passtime;
            }
            else
            {
                g_RemoteInfo.wOpenBrakeDelay[i] = 0;
            }
            if(g_RemoteInfo.wLimitEcTime[i] > passtime)
            {
                g_RemoteInfo.wLimitEcTime[i] -= passtime;
            }
            else
            {
                g_RemoteInfo.wLimitEcTime[i] = 0;
            }
            if(SWITCH_OPEN_STEP == g_RemoteInfo.bSwitchStep[i] || SWITCH_ALARM_STEP == g_RemoteInfo.bSwitchStep[i])
            {
                CTRL_FMT_DEBUG("轮次%d调整, 告警剩余时间 %02d:%02d, 限电剩余时间%02d:%02d\n", i + 1, 
                               g_RemoteInfo.wOpenBrakeDelay[i]/60, g_RemoteInfo.wOpenBrakeDelay[i]%60,
                               g_RemoteInfo.wLimitEcTime[i]/60, g_RemoteInfo.wLimitEcTime[i]%60);
            }
        }
...
* @Update    :
**********************************************************************/
void appctrl_remote_passday_init(uint8 nomax)
{
    uint8 i;
    //-过日自动恢复允许合闸状态-
    CTRL_FMT_DEBUG("过日/初始化 取消遥控告警和拉闸状态\n");
    for(i = 0; i < nomax; i++)
    {
        if(SWITCH_TYPE_OPEN == g_RemoteInfo.switchtype[i])
        {
...
int appctrl_get_RemotePara(uint8 no, uint8 nomax, CTRL_REMOTE_ARG_T *pArg)
{
    ...
    bFirst = get_remote_state(no + 1, &remoteType, &Trip, &Close);
    ...
    {
        pArg->trip.nNum = OOP_MAX_TRIP;
        pArg->trip.trip[no] = Trip;
        pArg->close.nNum = OOP_MAX_TRIP;
        pArg->close.close[no] = Close;
        ...
        g_RemoteInfo.remoteType[no] = remoteType;
    }
...
void appctrl_remote_deal(int cid, uint8 no, uint8 nomax, CTRL_REMOTE_ARG_T *pArg, CTRL_SUBGROUP_T *ptGroups)
{
    ...
    get_remote_state(no + 1, &remoteType, &Trip, &Close);
        ...
        case SWITCH_START_STEP:
        {
            ...
            {    //-拉闸命令-
                if (g_RemoteInfo.wOpenBrakeDelay[no] == 0)
                {
                    ...
                    set_remote_state_valid(g_RemoteInfo.turnNO[no] + 1 + OOP_MAX_TRIP / 2, 1, NULL, NULL);
                }
                else
                {
                    //-填写遥控显示信息结构-
...
void appctrl_remote_ctrl(int cid, uint8 bDayPass, uint8 nomax)
{
    ...
    if(TRUE == bDayPass)
    {
        appctrl_remote_passday_init(nomax);
    }
...
{
    int ret = 0;
    static REMOTE_INFO_T s_RemoteInfo = {0};
    ...
    ret = lctrl_write_ctrl(CTRL_REMOTE_TYPE, (uint8 *)&g_RemoteInfo, sizeof(REMOTE_INFO_T));
    if(0 != ret)
    {

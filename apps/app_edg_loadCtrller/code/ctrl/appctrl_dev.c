...
/**********************************************************************
* @name      : appctrl_write_zcu3_version
* @brief     ：写版本号
...
* @Update    :
**********************************************************************/
void appctrl_write_zcu3_version(char *version, uint16 len)
{
    int32 fd = 0;
    int32 iRet= 0;
    struct flock lock;

    fd = open("/data/app/stAmr/data/zcu_3_version", O_CREAT|O_WRONLY,0666);
    if(fd < 0)
    {
        CTRL_FMT_DEBUG("打开设备/data/app/rspSample/data/version失败\n");
        return;
    }
    ...
    if(iRet != 0)
    {
        CTRL_FMT_DEBUG("Get id Return value of fcntl:%d\n",iRet);
    }
    
    iRet = write(fd, version, len);
    if(iRet != len)
    {
        CTRL_FMT_DEBUG("写入状态失败\n");
    }
    ...
    if (iRet < 0)
    {
        CTRL_FMT_DEBUG("fcntl failed msg:%s\n", strerror(errno));
    }
...
/**********************************************************************
* @name      : appctrl_clean_zcu3_version
* @brief     ：清版本号
* @param[in] ：
* @param[out]：
* @return    ：
...
* @Update    :
**********************************************************************/
void appctrl_clean_zcu3_version()
{
    if(0 == access("/data/app/stAmr/data/zcu_3_version", F_OK))
    {
        remove("/data/app/stAmr/data/zcu_3_version");
    }
}

/**********************************************************************
* @name      : appctrl_get_zcu_3_version
* @brief     ：继电器消息收发
* @param[in] ：CTRL_RS232_T *pDevPara  设备句柄
               uint8 portNum           端口数量
* @param[out]：uint8 *input             接入配置
...
* @Update    :
**********************************************************************/
int appctrl_get_zcu_3_version(CTRL_RS232_T *pDevPara, char *version)
{
    uint8   datas[2] = {0};
    uint8   msglen = 0;
    uint8   Send_buf[200] = {0};
    ...
    if(0 == ioctl(pDevPara->fd, CONTROL_GET_MODSTATE, 0))
    {
        msglen = appctrl_switch_package(datas, 0XF1, Send_buf);
        ret = ctrl_uart_send(pDevPara, Send_buf, msglen);
        CTRL_BUF_DEBUG(Send_buf, msglen, "Ctrl %s send, ret %d", pDevPara->deviceName, ret);
        ...
        while (RetryCnt < 2)
        {
            ...
            if(ret >= 0)
            {
                version[0] = (ret & 0x000000FF);
                version[1] = ((ret >> 8) & 0x000000FF);
                return 0;
            }
            usleep(20000);
        }       
...
void appctrl_switch_init(uint8 port, uint8 turnMax)
{
    ...
    /* 获取文件的数据 */
    ret = lctrl_read_ctrl(CTRL_STATUS_TYPE, (uint8*)buf, 4096, &len);
...
int appctrl_set_switch_zcu3(uint8 acm1, uint8 subport1, SWITCH_INTERFACE_T *pInterface)
{
    ...
    if(TRUE == sswitchFirst)
    {
        char ver[3] = {0};
        ret = appctrl_get_zcu_3_version(&g_CtrlDev1, ver);
        if(0 == ret)
        {
            CTRL_FMT_DEBUG("获取版本号: %s\n", ver);
            appctrl_write_zcu3_version(ver, 2);
        }
        else
        {
            CTRL_FMT_DEBUG("获取版本号失败\n");
        }
    }
...
int appctrl_set_switch_close(SWITCH_INTERFACE_T *pInterface)
{
    ...
    static uint8                sAlarmStatus = 1;
    static uint8                bswitchAlarm = TRUE;
    struct timespec             curtime = {0};
    static uint32               ctrl_setAlarm = 0;
    
    clock_gettime(CLOCK_MONOTONIC, &curtime);
    /* 终端告警 */
    if(pInterface->ALARM_OUTPUT != sAlarmStatus || TRUE == bswitchAlarm || curtime.tv_sec >= ctrl_setAlarm + 60)
    {
        ret = appctrl_switch_alarm_out(pInterface->ALARM_OUTPUT);
        if(pInterface->ALARM_OUTPUT != sAlarmStatus || TRUE == bswitchAlarm)
        {
            CTRL_FMT_DEBUG("设置告警继电器%s动作%s\n", 0 == pInterface->ALARM_OUTPUT ? "" : "取消",
                                                       0 == ret ? "成功" : "失败");
        }
        sAlarmStatus = pInterface->ALARM_OUTPUT;
        bswitchAlarm = FALSE;
        ctrl_setAlarm = curtime.tv_sec;
    }
#endif
...
void appctrl_status_write(uint8 bDay)
{
    ...
    ret = lctrl_write_ctrl(CTRL_STATUS_TYPE, (uint8 *)g_BrakeBuf, OOP_MAX_TURN * sizeof(UN_BRAKE_DATA_T));
    if(0 != ret)
    {

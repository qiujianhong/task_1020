...
typedef struct str_AppLayerList{
    ...
    uint8 macflag;      //电表上报数据回码时用
    uint8 rnbuf[12];       //12字节随机数
...
typedef void (*OopConnectRequest_t)(uint8 ,OOP_TSA_T*,uint8,uint32,uint8*,uint8*) ;
...
typedef eOopCommRslt_t (*OopReportRequest_t)(AppLayerList_t*) ;
typedef void (*OopReportResponse_t)(uint8 ,OOP_TSA_T* ,uint8 ,uint8*,uint16) ;
...
extern eOopCommRslt_t Client_Resv_Apdu(uint8 pipe,LinkLayerAddr_t *a,LinkLayerCtrl_t ctrl,uint8 *apdu,uint16 apduLen);
extern uint16 LinkLayer_PackFrame(uint8 pipe,LinkLayerAddr_t *a,uint8*apdu,uint16 apduLen);
extern eOopCommRslt_t LinkDataReceive(uint8 pipe,const uint8 *Buffer, uint16 DataLength,OOP_TSA_T    *MtrCommAddr);
extern uint16 GetFrame(const uint8 *Buffer, uint16 DataLength, uint16 *OffSet);
extern eOopCommRslt_t LinkLayer_UnPackFrame(uint8 pipe,uint8 *inframe,uint16 inLen,OOP_TSA_T    *MtrCommAddr);
...
//connect
extern void ConnectRequest(uint8 pipe,OOP_TSA_T *sa,uint8 connectId,uint32 timeOut,uint8* secret,uint8* sign);
...
//Security
extern void SecurityRequest(uint8 pipe,uint8 *inData,uint16 inLen);
extern eOopCommRslt_t SecurityConfirm(AppLayerList_t *appLayerList);

//report
extern void ReportResponse(uint8 pipe,OOP_TSA_T *sa,uint8 nChoice,uint8 *inData,uint16 inLen);
extern eOopCommRslt_t ReportRequest(AppLayerList_t *appLayerList);

extern void security_rand_get();
//服务层对外接口
extern const OopGet_t OopGet;
extern const OopSet_t OopSet;
extern const OopAction_t OopAction;
extern const OopReport_t OopReport;
...
//协议数据编码解码等接口
extern void OopDataEncode(enum tag_OOP_DATATYPE dataType,uint16 dataLen,uint8 *pSrcBuff,uint8 *pDesBuff,uint16 *pOffset);
...
extern uint8 getDataTypeLength(enum tag_OOP_DATATYPE dataType);
...
extern int8 Init_ReadMtr(uint8 pipe);
...
extern int8 TESAM_RcvSndDataAndCheck(uint8 *sndData, uint16 sndLen, uint8 *pOutBuf, uint16 *rcvLen);

int security_rand1_get(uint8* buf);
int security_sessondata1_get(uint8* inbuf,uint16 inlen,uint32 counter,uint8* outbuf);
int security_sessonkey_get(uint8* key,uint16 keylen,uint8* sessiondata2,uint16 sessiondata2len,uint8* mac2,uint16 mac2len,uint8* outbuf);
...
int security_sessonkey_get_698(uint8* key,uint16 keylen,uint8* sessiondata2,uint16 sessiondata2len,uint8* mac2,uint16 mac2len,uint8* outbuf);

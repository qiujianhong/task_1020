...
static sint32 thread_safety_call(SAFETY_CALL_FP_T func, void *obj)
{
    ...
    {
        MQTTMSG_FMT_DEBUG("thread_safety_call fail\n");
        return -1;
    }
    return 0;
}
...
static void i_disconnect_client(sint8 *ip, uint16 port)
{
    ...
    if (data != NULL)
    {
        ...
        // MQTTMSG_FMT_DEBUG("remove tcp client[%s] from pool\n", key);
        MQTTMSG_FMT_DEBUG("remove tcp client[%s] from pool\n", key);
...
static void tcp_msg_handel_thread_safety_cb(void *obj)
{
    ...
    // MQTTMSG_FMT_DEBUG("===>call %s in %s\n", "tcp_msg_handel_thread_safety_cb",name);
...
static void client_buffered_on_read(struct bufferevent *bev, void *arg)
{
    ...
    do
    {
        ...
        if (ctx == NULL)
        {
            MQTTMSG_FMT_DEBUG("TCP_CLIENT_CTX_T calloc fail\n");
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("tcp recv data[%zu] < MIN_RAW_BUF_LEN[%d]", ebTotallen,
                       MIN_RAW_BUF_LEN);
            break;
        }

        MQTTMSG_FMT_DEBUG("recv ebTotallen:[%zu] from [%s]:[%u]", ebTotallen,
        ...
        {
            MQTTMSG_BUF_DEBUG(rxBuf, rxBufLen);
            msgHanderObj =
    ...
    do
    {
        ...
        MQTTMSG_FMT_DEBUG("tcp rxBufLen =%d\n", rxBufLen);
        ...
        else
        {
            ...
            {
                static uint32 iii = 0;
                MQTTMSG_FMT_DEBUG("tcp 0x11111111 =%d\n", ++iii);
            }

            if (rxBufLen < 259)
            {
                MQTTMSG_BUF_DEBUG(ctx->reqRawObj.payload, rxBufLen,"tcp ebTotallen =%zd", ebTotallen);
            }

            MQTTMSG_BUF_DEBUG(rxBuf, rxBufLen, "tcp msg\n");
            msgHanderObj =
                (MSG_HANDER_OBJ_T *)calloc(1, sizeof(MSG_HANDER_OBJ_T));

            if (msgHanderObj == NULL)
            {
                MQTTMSG_FMT_DEBUG("TCP_CLIENT_CTX_T calloc msgHanderObj fail\n");
                free(ctx);
                break;
            }
            ...
            if (ebTotallen > MIN_RAW_BUF_LEN)
            {
                MQTTMSG_FMT_DEBUG("######## recv ebTotallen:[%zu]\n", ebTotallen);
                continue;
            }
        }
        printf("Drained %lu\n", (unsigned long)ebTotallen);
...
static void client_buffered_on_write(struct bufferevent *bev, void *arg)
{
    (void)bev;
    UNUSED(arg);
    MQTTMSG_FMT_DEBUG("buffered_on_write\n");
}
...
client_buffered_on_error(struct bufferevent *bev, short what, void *arg)
{
    ...
    if (what & BEV_EVENT_EOF)
    {
        ...
        MQTTMSG_FMT_DEBUG("Client disconnected [%s]:[%u].", client->clientIP,
                   client->clientPort);
    }
    else
    {
        MQTTMSG_FMT_DEBUG("Client socket error, disconnecting [%s]:[%u].",
                   client->clientIP, client->clientPort);
    }
...
static void on_accept(int fd, short ev, void *arg)
{
    ...
    do
    {
        ...
        if (clientFd < 0)
        {
            MQTTMSG_FMT_DEBUG("accept failed fd[%d]  error:%s\n", fd, strerror(errno));
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("failed to set client socket non-blocking error:%s",
                       strerror(errno));
            break;
        }
        ...
        if (client == NULL)
        {
            MQTTMSG_FMT_DEBUG("malloc client failed error:%s\n", strerror(errno));
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("bufferevent_socket_new tcp fail\n");
            break;
        }
        ...
        MQTTMSG_FMT_DEBUG("Accepted connection from [%s]:[%d]", client->clientIP,
                   client->clientPort);
        ...
        if (ret != 0)
        {
            MQTTMSG_FMT_DEBUG("%s put tcp connect pool\n", key);
        }
        else
        {
            /* 内存不足反向释放所有数据 */
...
static sint32 tcp_sock_init(void)
{
    ...
    do
    {
        ...
        {
            MQTTMSG_FMT_DEBUG("create tcp socket fial\n");
            break;
        }
        MQTTMSG_FMT_DEBUG("s_tcpServer->listenFd  = %d\n", s_tcpServer->listenFd);
        ...
        {
            MQTTMSG_FMT_DEBUG("setsockopt SO_REUSEADDR  s_tcpServer->listenFd fial\n");
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("bind socket:[%d] fial ip:[%s] port:[%d]",
        ...
        {
            MQTTMSG_FMT_DEBUG("TCP server bind on ip:[%s] port:[%u]",
        ...
        {
            MQTTMSG_FMT_DEBUG("setsockopt s_tcpServer->listenFd fial\n");
            break;
        }

        if (listen(s_tcpServer->listenFd, TCP_LISTEN_NUM) < 0)
        {
            MQTTMSG_FMT_DEBUG("listen failed\n");
            break;
        }

        if (setnonblock(s_tcpServer->listenFd) < 0)
        {
            MQTTMSG_FMT_DEBUG("setnonblock socket:[%d] fial\n", s_tcpServer->listenFd);
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("s_tcpServer->connectPool init fial\n");
            break;
        }
...
static sint32 libevent_init(void)
{
    ...
    do
    {
        ...
        if (base == NULL)
        {
            MQTTMSG_FMT_DEBUG("event_base_new fail\n");
            break;
        }
        ...
        if (0 != event_assign(&s_tcpServer->eventAccept, base,
                              ...
                              on_accept, NULL))
        {
            MQTTMSG_FMT_DEBUG("event_assign fail\n");
            break;
        }

        if (0 != event_add(&s_tcpServer->eventAccept, NULL))
        {
            MQTTMSG_FMT_DEBUG("event_add fail\n");
            break;
        }
...
sint32 tcp_init_system(void)
{
    ...
    do
    {
        INIT_FMT_DEBUG("tcp_init_system\n");
        s_tcpServer = (TCP_SERVICE_T *)calloc(1, sizeof(TCP_SERVICE_T));
        if (s_tcpServer == NULL)
        {
            INIT_FMT_DEBUG("malloc TCP_SERVICE_T fail\n");
            break;
        }
...
{
    MQTTMSG_FMT_DEBUG("tcp_clean_up_system\n");
}
...
{
    sint32 ret = -1;
    MQTTMSG_FMT_DEBUG("tcp_start_system\n");

    do
    {
        if (0 != init_pipe())
        {
            MQTTMSG_FMT_DEBUG("tcp server init pipe file\n");
            break;
        }

        if (0 != tcp_sock_init())
        {
            MQTTMSG_FMT_DEBUG("tcp_sock_init fail\n");
            break;
        }

        if (0 != libevent_init())
        {
            MQTTMSG_FMT_DEBUG("libevent_init fail\n");
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("pthread_create s_tcpServer fail\n");
            break;
        }
        ret = 0;
...
{
    MQTTMSG_FMT_DEBUG("tcp_stop_system\n");
}
...
sint32 tcp_sendto(sint8 ip[IP_LEN], uint16 port, void *data, uint32 dataLen)
{
    ...
    if (val != NULL)
    {
        ...
        {
            MQTTMSG_FMT_DEBUG("calloc tcp_sendto cmd fail\n");
        }
    }

...
static void timeout_cb(evutil_socket_t fd, sint16 event, void *arg)
{
    ...
    else
    {
        ...
        MQTTMSG_FMT_DEBUG("udp server stop now\n");
    }
}
...
static void udp_cb(const sint32 sock, sint16 which, void *arg)
{
    ...
    do
    {
        ...
        if (NULL == ctx)
        {
            MQTTMSG_FMT_DEBUG("alloc UDP ctx fial\n");
            break;
        }
        ...
        {
            MQTTMSG_FMT_DEBUG("UDP recv packetlen:[%d] < MIN_RAW_BUF_LEN\n", recvLen);
            break;
        }
#ifdef XL_TIME_TRACE
        getSysTime(&recvDone);
        ...
        else
        {
            ...
            {
                MQTTMSG_FMT_DEBUG("queue msg fail\n");
            }
...
static void *udp_service(void *arg)
{
    ...
    {
        MQTTMSG_FMT_DEBUG("UDP server Bind Port:[%d] fail.\n", s_udpServer->serverPort);
        exit(EXIT_FAILURE);  //端口绑定失败退出程序
    }
    else
    {
        MQTTMSG_FMT_DEBUG("UDP server bind on ip:[%s] port:[%u]",
...
sint32 udp_init_system()
{
    ...
    INIT_FMT_DEBUG("udp_init_system\n");
...
{
    MQTTMSG_FMT_DEBUG("udp_clean_up_system\n");
}
...
{
    sint32 ret = -1;
    MQTTMSG_FMT_DEBUG("udp_start_system\n");
    //启动UDP线程
...
{
    MQTTMSG_FMT_DEBUG("udp_stop_system\n");
}
...
sint32 udp_sendto(sint8 ip[IP_LEN], uint16 port, void *data, uint32 dataLen)
{
    ...
    MQTTMSG_FMT_DEBUG("send_to %s:%u sendLen:[%u]\n", ip, port, sendLen);
    if  (sendLen != -1) {
        sendLen = 0;
    }
    return sendLen;

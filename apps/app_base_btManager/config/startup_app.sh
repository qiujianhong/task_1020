#!/bin/sh
PATH=$PATH:/usr/local/extapps/btManager/bin
export PATH
...
mkdir -p $dataShardpath/btManager
mkdir -p $dataShardpath/btManager/log
mkdir -p $dataShardpath/btManager/data
mkdir -p $dataShardpath/btManager/cfg

# 文件系统同步
sync;sync

chmod 755 /usr/local/extapps/btManager/bin/btManager

/usr/local/extapps/btManager/bin/btManager  >/dev/null  2>&1 &
echo "btManager program started!"

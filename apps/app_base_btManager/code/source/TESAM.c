...
//gTesamDev句柄只创建一次，添加相关全局变量
static ESAM_DEVICE_T*  gTesamDev = NULL;                     
...
ESAM_DEVICE_T* esam_device_get(const char *device_id)
{
    if (!isInit)
    {
        gTesamDev = (ESAM_DEVICE_T*)hal_device_get(device_id);
        if (gTesamDev != NULL)
        {
            ...
            gTesamDev->esam_set_power(gTesamDev, PWR_ON);
            ...
            isInit = TRUE;
            TESAM_FMT_TRACE("esam_device_get ok\n");
        }
        else
        {
            TESAM_FMT_TRACE("esam_device_get err\n");
        }
    }
    return gTesamDev;
}
...
int Init_Fd_Esam(void)
{
    ...
    else
    {
        ...
        dev = (ESAM_DEVICE_T*)esam_device_get(HW_DEVICE_ID_YX_ESAM);
        if (dev == NULL)
        {
            return NO_ESAM;
        }

        TESAM_FMT_TRACE("g_fd_esam = %p\n", dev);
    }
    if(ESAM_FAIL == GetTEsam_Info(0xFF,g_TESAMInfoBuff,&g_TESAMInfoLen))
    {
        g_TESAMInfoLen = 0;
        esam_device_release();
        TESAM_FMT_TRACE("GetTEsam_Info err\n");
        return NO_ESAM;
    }
    return 0;
}
...
int8 TESAM_RcvSndDataAndCheckUnit(uint8 *sndData, uint16 sndLen, uint8 *pOutBuf, uint16 *rcvLen, BOOL needReverse)
{
    ...
    else
    {
        ...
        while (i++)
        {
            ...
            if (i > 100)
            {
                TESAM_FMT_DEBUG("dev->esam_set_lock failure %d \n", iRet);
                return ESAM_FAIL;
            }
        }
        ...
        if(iRet < 0)
    	{
    	  TESAM_FMT_DEBUG("dev->esam_data_write Send data failure %d \n", iRet);
          iRet = dev->esam_set_lock(dev, 0);
          if (iRet != ERR_OK)
          {
              TESAM_FMT_DEBUG("dev->esam_set_unlock failure %d \n", iRet);
          }
    	  return ESAM_FAIL;
    	}
        ...
        if(iRet < 0)
    	{
    	  TESAM_FMT_DEBUG("dev->esam_data_read recv data failure %d \n", iRet);
          iRet = dev->esam_set_lock(dev, 0);
          if (iRet != ERR_OK)
          {
              TESAM_FMT_DEBUG("dev->esam_set_unlock failure %d \n", iRet);

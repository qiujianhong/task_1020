...
// 对外消息 客户端句柄
MQTT_CLIENT_T *g_BLEMsgClient;

#define BLE_SUB_TOPIC      "+/M-btManager/#"
#define BLE_SUB1_TOPIC     "+/btManager/#"

pthread_mutex_t   g_BleSendlock; 
pthread_mutex_t   g_BleIndexlock; 
uint16            g_BleMsgindex = 0;
...
int ble_mqtt_init(uint32 ip, uint16 port)
{
    ...
    /* 新建客户端 */
    sprintf((char *)g_AppName, "%s", BLE_APP_NAME);
    sprintf((char *)g_AppVer, "%s", APP_VERSION);
    g_BLEMsgClient = mqtt_client_create(BT_APP_NAME);
    if (NULL == g_BLEMsgClient)
    {
        BLE_FMT_DEBUG("mqtt_client_create fail\n");
        return -1;
    }
    ...
    subtopic[0].qos = 0;
    memcpy(subtopic[0].topic, BLE_SUB_TOPIC, strlen(BLE_SUB_TOPIC));
    subtopic[1].qos = 0;
    memcpy(subtopic[1].topic, BLE_SUB1_TOPIC, strlen(BLE_SUB1_TOPIC));

    ret = mqtt_client_set_sub_topic(g_BLEMsgClient, subtopic, 2);
    if(0 != ret)
    {
        BLE_FMT_DEBUG("mqtt_client_set_sub_topic fail, ret %d\n", ret);
        mqtt_client_destroy(g_BLEMsgClient);
        return ret;
    }

    /* 链接服务端 */
    BLE_FMT_DEBUG("链接mqtt服务端 %s:%d\n", mqttaddr, port);
    ret = mqtt_client_connect(g_BLEMsgClient, mqttaddr, port, NULL);
    if(0 != ret)
    {
        BLE_FMT_DEBUG("链接mqtt服务端链接失败 ret %d\n", ret);
        mqtt_client_destroy(g_BLEMsgClient);
        return ret;
    }
    ...
    while(1)
    {
        status = mqtt_client_get_status(g_BLEMsgClient);
    ...
    BLE_FMT_DEBUG("链接mqtt服务端成功\n");

    pthread_mutex_init(&g_BleSendlock, NULL);
    pthread_mutex_init(&g_BleIndexlock, NULL);
    return 0;
}

/**********************************************************************
* @name      : ble_get_next_index
* @brief     ：对外消息index分配
* @param[in] ：
* @param[out]：
* @return    ：index
...
uint16 ble_get_next_index()
{
    ...
    PTHREAD_LOCK_SAFE((void *)&g_BleIndexlock);
    curindex = g_BleMsgindex;
    g_BleMsgindex++;
    PTHREAD_UNLOCK_SAFE((void *)&g_BleIndexlock);
...
int ble_send_axdr_msg(MESSAGE_INFO_T *pMsgAXDR, int qos)
{
    ...
    PTHREAD_LOCK_SAFE(&g_BleSendlock);
    ret = mqtt_client_send_msg(g_BLEMsgClient, topic, qos, pMsg, (int)msglen);
    PTHREAD_UNLOCK_SAFE(&g_BleSendlock);
...
int ble_send_json_msg(char *topic, int qos, char *pMsg, int msglen)
{
    ...
    PTHREAD_LOCK_SAFE(&g_BleSendlock);
    ret = mqtt_client_send_msg(g_BLEMsgClient, topic, qos, pMsg, msglen);
    PTHREAD_UNLOCK_SAFE(&g_BleSendlock);
...
{
    int qos;
    return mqtt_client_recv_msg(g_BLEMsgClient, pTopic, topiclen, &qos, pBuf, (int)buflen, 1000);
}

/**********************************************************************
* @name      : ble_login_to_sys
* @brief     ：APP注册
* @param[in] ：
* @param[out]：
* @return    ：0-成功/错误码
...
int ble_login_to_sys()
{
    ...
    tAppReg.name.num = strlen(BT_APP_NAME);
    memcpy(tAppReg.name.value, BT_APP_NAME, tAppReg.name.num);
    tAppReg.version = ble_stoi(APP_VERSION);
    ...
    token = ble_get_next_index();
    msglen = sys_appreg_package(token, &tAppReg, msg, 256);
    if(0 == msglen)
    {
        return 1;
    }
    sprintf(topic, "%s/%s/%s/%s/%s", BLE_APP_NAME, SYS_APP_NAME, MQTT_TOPIC_JSON, MQTT_TOPIC_INFO_REQ, SYS_TOPIC_REGISTER);
    ret = ble_send_json_msg(topic, 0, msg, msglen);
    return ret;
}
...
int ble_send_errmsg(MESSAGE_INFO_T *pRecvMsg, int16 errCode)
{
    ...
    ptAXDR->sourlen = strlen(BT_APP_NAME);
    memcpy(ptAXDR->souraddr, BT_APP_NAME, ptAXDR->sourlen);
...
int ble_err_json_res(MESSAGE_JSON_T *pInMsg, uint8 errcode, char *err)
{
    ...
    if(pMsg)
    {
        ...
        ret = ble_send_json_msg(topic, 0, pMsg, strlen(pMsg));
        free(pMsg);
    }
    return ret;
}

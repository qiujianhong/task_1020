...
int mqtt_init()
{
    ...
    {
        sprintf(topic[2].topic, "M-mapManager/Broadcast/0004/#");
        topic[2].qos = 0;
        topicNum++;
    }
...
void heart_beat(uint16 index)
{
	...
	//打印完整消息内容
	CCOUP_FMT_TRACE("heart_beat() 心跳回复...通过MQTT发送的消息为 (topic:%s) \n", topic);
	//PrintInfoDetail(MsgSend);
	CCOUP_FMT_TRACE("Send Up IPC data... SendLen=%d \n", SendLen);
	CCOUP_BUF_TRACE(SendBuf, SendLen, "发送mqtt报文:");
    mqtt_client_send_msg(g_MqttClient,topic,0,SendBuf, SendLen);
    free(MsgSend);
...
void event_to_smiOS(uint8 *pPayloadData, uint32 nPayloadLen)
{
    ...
    //打印完整消息内容
    CCOUP_FMT_TRACE("event_to_smiOS() 事件上报smiOS...通过MQTT发送的消息为 (topic:%s) \n", topic);
    //PrintInfoDetail(MsgSend);
    CCOUP_FMT_TRACE("Send Up IPC data... SendLen=%d \n", SendLen);
    CCOUP_BUF_DEBUG(SendBuf, SendLen, "发送mqtt报文:");
    mqtt_client_send_msg(g_MqttClient,topic,0,SendBuf, SendLen);
...
int mqtt_topic_send_msg(char *pTopic, MESSAGE_INFO_T *pMsgInfo, int qos)
{
    ...
    if(0 == ret)
    {
        CCOUP_BUF_TRACE(pMsg, msglen, "Send msg:");
    }
    else
    {

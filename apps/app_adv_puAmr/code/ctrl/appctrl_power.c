...
void appctrl_power_init()
{
    ...
    ret = appctrl_read_ctrl(CTRL_POWER_TYPE, (uint8*)buf, 4096, &len);
...
void appctrl_power_status(uint8 turnmax)
{
    ...
    for(grp = 0; grp < OOP_MAX_TG; grp++)
    {
        ...
        curCtrlStatus.pwFall = ptCtrlArg->tDown.project[grp].scheme.floatCoefficient;
...
uint8 appctrl_down_guishow(uint8 group, uint8 turnmax, char* pBuf, uint16 buflen, uint16* msglen, int pinx)
{
    ...
    *msglen = offset;
    ...
    ctrlInfo.group = group + 1;
    ctrlInfo.type = CTRL_DOWN_TYPE;
    ctrlInfo.times = 65;

    sprintf(ctrlInfo.head, "%s", "下浮控");
    if(CTRL_DOWN_TYPE != g_PowerInfo.bPcKind[group])
    {
        sprintf(ctrlInfo.info, "定值: ---- kW\r\n滑差功率: %lld.%04lldkW\r\n当前功率: %lld.%04lldkW\r\n", 
                           //g_PowerInfo.wPowerSetValue[group]/10000, g_PowerInfo.wPowerSetValue[group]%10000,
                           ptSubGroups[group].aveActPower/10000, ptSubGroups[group].aveActPower%10000,
    ...
    {
        sprintf(ctrlInfo.info, "定值: %lld.%04lldkW\r\n滑差功率: %lld.%04lldkW\r\n当前功率: %lld.%04lldkW\r\n", 
                           g_PowerInfo.wPowerSetValue[group]/10000, g_PowerInfo.wPowerSetValue[group]%10000,
                           ptSubGroups[group].aveActPower/10000, ptSubGroups[group].aveActPower%10000,
                           ptSubGroups[group].actPower/10000, ptSubGroups[group].actPower%10000);
    }
    ...
    for(i = 0; i < turnmax; i++)
    {
        if(CTRL_EFFECT != g_PowerInfo.bPcTurnSet[group][i] || FALSE == appctrl_dev_input(i + 1))
        {
            continue;
        }
        ...
        else
        {
            ...
            if(0 == g_PowerInfo.wPcAlarmDelayss[group][i])
            {
                ctrlInfo.turn[ctrlInfo.num].futurestatus = 0;
            }
            else
            {
                ctrlInfo.turn[ctrlInfo.num].futurestatus = 1;
        ...
        ctrlInfo.turn[ctrlInfo.num].min = g_PowerInfo.wPcAlarmDelayss[group][i] / 60;
        ctrlInfo.turn[ctrlInfo.num].sec = g_PowerInfo.wPcAlarmDelayss[group][i] % 60;
...
//                      ptSubGroups[group].actPower/10000, ptSubGroups[group].actPower%10000);
    ...
    *msglen = offset;
    ...
    ctrlInfo.group = group + 1;
    ctrlInfo.type = CTRL_STOP_TYPE;
    ctrlInfo.times = 65;

    sprintf(ctrlInfo.head, "%s", "报停控");
    if(CTRL_STOP_TYPE != g_PowerInfo.bPcKind[group])
    {
        sprintf(ctrlInfo.info, "定值: ---- kW\r\n滑差功率: %lld.%04lldkW\r\n当前功率: %lld.%04lldkW\r\n",
                           ptSubGroups[group].aveActPower/10000, ptSubGroups[group].aveActPower%10000,
...
uint8 appctrl_rest_guishow(uint8 group, uint8 turnmax, char* pBuf, uint16 buflen, uint16* msglen, int pinx)
{
    ...
    *msglen = offset;
    ...
    ctrlInfo.group = group + 1;
    ctrlInfo.type = CTRL_REST_TYPE;
    ctrlInfo.times = 65;

    sprintf(ctrlInfo.head, "%s", "厂休控");
    if(CTRL_REST_TYPE != g_PowerInfo.bPcKind[group])
    {
        sprintf(ctrlInfo.info, "定值: ---- kW\r\n滑差功率: %lld.%04lldkW\r\n当前功率: %lld.%04lldkW\r\n",
                           ptSubGroups[group].aveActPower/10000, ptSubGroups[group].aveActPower%10000,
...
uint8 appctrl_period_guishow(uint8 group, uint8 turnmax, char* pBuf, uint16 buflen, uint16* msglen, int pinx)
{
    ...
    *msglen = offset;
    ...
    ctrlInfo.group = group + 1;
    ctrlInfo.type = CTRL_PERIOD_TYPE;
    ctrlInfo.times = 60;

    sprintf(ctrlInfo.head, "%s", "时段控");
    if(CTRL_PERIOD_TYPE != g_PowerInfo.bPcKind[group])
    {
        sprintf(ctrlInfo.info, "定值: ---- kW\r\n滑差功率: %lld.%04lldkW\r\n当前功率: %lld.%04lldkW\r\n", 
                           ptSubGroups[group].aveActPower/10000, ptSubGroups[group].aveActPower%10000,
...
void appctrl_power_write(uint8 bDay)
{
    ...
    ret = appctrl_write_ctrl(CTRL_POWER_TYPE, (uint8 *)&g_PowerInfo, sizeof(POWER_INFO_T));
    if(0 != ret)
    {

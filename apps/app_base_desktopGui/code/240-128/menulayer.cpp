...
{
	return (dwID==0x20A00000||dwID==0x20A10000||dwID==0x20A20000)?TRUE:FALSE;
}
...
char ac_minver[100] = {0};
char ac_data_ver[100] = {0};
char ac_ver[100] = {0};
char ac_meterIC_ver[100] = {0};
...
void ShowOrHideMenu()
{
    ...
    g_bpop = FALSE;
    get_ac_minver(ac_minver);
    get_ac_data_ver(ac_data_ver);
    get_ac_ver(ac_ver);
    get_ac_meterIC_ver(ac_meterIC_ver);
	if(g_bAllMenu == TRUE)
	{
...
//    GUI_FMT_TRACE("g_nParentIndex %d \n", g_nParentIndex);
	// 根据父菜单id生成子菜单项
	g_nMaxMenuChar = 0;
	int nTmpChar;
...
void MenuDraw(HDC hdc)
{
	...
	for (i = g_menuBase.nMenuShowMin; i <= g_menuBase.nMenuShowMax; i++)
	{
		...
		rc.top = Top;
		rc.bottom = Top + 16;
//		sprintf(czBuf, "%s", pMenu->czCaption);						//集中器的主菜单不带序号
		sprintf(czBuf, "%d.%s", i + 1, pMenu->czCaption);
...
void MenuOkKeyMsg()
{
    ...
    GUI_FMT_TRACE("g_nCurSelect %d nIndex %d, g_nCurMenuCnt %d\n", g_nCurSelect, nIndex, g_nCurMenuCnt);
...
BOOL TwoMenuKeyMsg(int nKey)
{
    ...
    case KEY_RIGHT:
		{
            // 菜单选择项上移
			if((g_nCurSelect+1) > (g_menuTwo.nMenuShowMin+4))
						{
                g_nCurSelect=g_nCurSelect-4;
            }
			else
			{
				if(g_nCurMenuCnt>(g_nCurSelect+4))
					g_nCurSelect=g_nCurSelect+4;
			}
				g_narrMenuIndex[g_nMenuNodes-1] = g_nCurSelect;
                UpdateWindow(g_guiComm.hMainWnd, TRUE);
...
void ThreeInitMenu(DWORD dwPluginId)
{
	...
	if(g_nParentIndex != -1)
	{
		...
		g_nTop = pMenu->nTop;
		g_nInterval = pMenu->nInterval;
		g_menuThree.nMenuShows = pMenu->nMenuMaxCnt/4*4;
	}
	else
	{
		g_nLeft1 = g_guiMain.nLeft1;
		...
		g_nTop = g_guiMain.nTop;
		g_nInterval = g_guiMain.nMenuInterval;
		g_menuThree.nMenuShows = g_guiMain.nMenuMaxCnt/4*4;
	}
...
//		printf("[0007] %s---%s \n",czBuf,showName);
		...
		x = 16 + ((i-g_menuThree.nMenuShowMin) % 4)*50;
		y = 25 + ((i-g_menuThree.nMenuShowMin) / 4)*54;
		width = 32;
		height = 32;
...
//						g_nCurSelect,g_menuThree.nMenuShowMin,g_menuThree.nMenuShowMax,g_nCurMenuCnt);
					if(g_menuThree.nMenuShowMax%4 == 0 && g_menuThree.nMenuShowMax < g_nCurMenuCnt-1)
					{
						g_menuThree.nMenuShowMax ++;
					}
				}
			...
			else
			{
				if(g_nCurSelect == 0)
				{
					g_nCurSelect = (g_nCurMenuCnt-1)/4*4;
				}
				else if(g_nCurSelect == 1)
				{
					g_nCurSelect = (g_nCurMenuCnt-1)/4*4+1;
					if(g_nCurSelect > g_nCurMenuCnt-1)
					{
						g_nCurSelect -= 4;
					}
				}
                else if(g_nCurSelect == 2)
				{
					g_nCurSelect = (g_nCurMenuCnt-1)/4*4+2;
					if(g_nCurSelect > g_nCurMenuCnt-1)
					{
						g_nCurSelect -= 4;
					}
				...
				{
					g_nCurSelect = (g_nCurMenuCnt-1)/4*4+3;
					if(g_nCurSelect > g_nCurMenuCnt-1)
					{
						g_nCurSelect -= 4;
					}
				...
				{// 如果总菜单项已经大于能够显示菜单项的最大值
                    ...
                    else if(g_nCurMenuCnt%g_menuThree.nMenuShows < 4)
                    {
						g_menuThree.nMenuShowMin = g_nCurMenuCnt - g_nCurMenuCnt%g_menuThree.nMenuShows - 4;
					}
                    else
                    {
	...
	case KEY_DOWN:
		{
            // 菜单选择项下移
			if(g_nCurSelect+4 < g_nCurMenuCnt)
			{
				g_nCurSelect += 4;
                // 调整菜单显示区域，如果菜单项不能全显的情况下，
                // 选择项已经大于最大显示菜单项，将显示区域下移

...
/**********************************************************************
* @name      : gui_send_extend_wirelessmsg
* @brief     ：gui对外发送无线拨号app消息接口(扩展)
* @param[in] ：uint16          label
               uint16          iop
               uint8*          playload
...
/*int gui_send_extend_wirelessmsg(uint16 label, uint16 iop, uint8* playload, uint16 length)
{
    ...
    pMsgInfo->IID = MESSAGE_IID_WIRELESSGUI;
    pMsgInfo->IOP = iop;
...
* @Update    :
**********************************************************************/
int gui_send_modmsg(uint16 label, uint16 iop, uint8* playload, uint16 length)
{
    MESSAGE_INFO_T *pMsgInfo = NULL;
    uint16          infolen = 0;
    int             ret = 0;
...
BOOL gui_recv_regapp(MESSAGE_INFO_T *pInMsg)
{
    ...
    if(g_APPdbCenter.Online == FALSE)
    {
        gRetry++;
        GUI_FMT_DEBUG("Retry = %d\n", gRetry);
        printf("get dbcenter process....\n");
    }
    ...
    GUI_FMT_TRACE("dbCenter = %x, wireless = %x, Cts = %x, Mod = %x\n", g_APPdbCenter.Online, g_APPWireless.Online, 
                                                                        g_APPCTS.Online, g_APPMod.Online);

#ifdef PRODUCT_ECU
    if(TRUE == g_APPdbCenter.Online && TRUE == g_APPWireless.Online && TRUE == g_APPMod.Online
        && TRUE == g_APPCTS.Online)
#else
    if(TRUE == g_APPdbCenter.Online && TRUE == g_APPWireless.Online && TRUE == g_APPCTS.Online)
#endif
    {
        return TRUE;
    }

...
uint8 getnetmodeflag=0;
int8 getpositionflag=0;
OOP_GEOGADD_A_T GpsPosition;
...
int task_gui_recv_cts(MESSAGE_INFO_T *pMsgInfo)
{
    ...
    if(pMsgInfo->IOP == CTS_IOP_READ_ONE_REQ)
    {
...
int deal_at_simsel(uint8 *dataBuf)
{
    char *ptemp;
    if((ptemp = strstr((char*)dataBuf, "^SIMSEL:")) != NULL)
    {
        netmode = ptemp[9]-'0';
		getnetmodeflag=1;
    }
...
int task_comm_recv_wire(MESSAGE_INFO_T *pMsgInfo)
{
	...
	switch(pMsgInfo->IOP)
	{
		...
		case WIRELESS_IOP_TRANS_AT://公专模式
		{
			ret =deal_at_simsel(pMsgInfo->playload);
            return ret;
		}	
        case WIRELESS_IOP_GET_GPS://地理位置信息
		{
		    GPS_T tGps={0};
			ret =appmsg_gps_split(pMsgInfo->playload, &tGps);
            if(ret==0)
            {
                getpositionflag = 1;
                GpsPosition.type = 1;
                GpsPosition.newaddr.height = tGps.position.height;
                GpsPosition.newaddr.longitude.position = tGps.position.longitude.position;
                GpsPosition.newaddr.longitude.degre = tGps.position.longitude.degre;
                GpsPosition.newaddr.longitude.branch = tGps.position.longitude.branch;
                GpsPosition.newaddr.latitude.position = tGps.position.latitude.position;
                GpsPosition.newaddr.latitude.degre = tGps.position.latitude.degre;
                GpsPosition.newaddr.latitude.branch = tGps.position.latitude.branch;
            }
            return ret;
		}	
		default: return TASK_UNKNOW_MSG;

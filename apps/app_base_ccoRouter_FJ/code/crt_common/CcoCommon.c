...
//档案列表全局变量
AMR_METER_FILES_LIST_T gAmrMeterFilesListTemp[MAX_DOWN_THREAD_NUM] = {0};  //档案列表临时变量(消息缓存临时变量)
AMR_METER_FILES_LIST_T gAmrMeterFilesList[MAX_DOWN_THREAD_NUM] = {0};  //档案列表全局变量
AMR_METER_FILES_LIST_T gToAddFilesList[MAX_DOWN_THREAD_NUM] = {0};  //需要添加的档案列表全局变量
AMR_METER_FILES_LIST_T gToDeleteFilesList[MAX_DOWN_THREAD_NUM] = {0};  //需要删除的档案列表全局变量
...
int fd_plc[MAX_DOWN_THREAD_NUM] = {0};//plc复位引脚和模块插入引脚检测文件描述符
...
#ifdef AREA_FUJIAN
uint32  g_CcoMode = CCO_MODE_FUJIAN;  /* 1:国网,2:福建 */
#else
uint32  g_CcoMode = CCO_MODE_GW;  /* 1:国网,2:福建 */
#endif
...
*******************************************************************************/
#define CCO_UPDATE_PKG_LEN  512 /* 升级报文携带的文件长度 */
UpdateSpecialProcess UpdateFileSpecialDeal(uint8 nArrIndex)
{
    ...
    if(MMID_NULL == gModuleManuID[nArrIndex])
        tmpUpdateSpecialProcess.packageLen       = CCO_UPDATE_PKG_LEN;
    else
    {
        tmpUpdateSpecialProcess.packageLen = gLocalCommModuleInfo[nArrIndex].FileTransferMaxSolePackageLen;
        //针对载波模块上报的升级报文单包长度是大于512字节的，默认为512
        if(gLocalCommModuleInfo[nArrIndex].FileTransferMaxSolePackageLen > CCO_UPDATE_PKG_LEN)
            tmpUpdateSpecialProcess.packageLen       = CCO_UPDATE_PKG_LEN;
    }
...
int SlaveMonitorDeal(TASK_FUN_INPUT stTaskFunInput)
{
	...
	static INFO_DATA_HEAD_T InfoDataHead[MAX_DOWN_THREAD_NUM];  //消息原始HEAD
#if STAT_FUN
    static struct timeval downsendtv[MAX_DOWN_THREAD_NUM];
#endif
	...
	case INNER_RTS_SEND: //发送
		...
		{
#if STAT_FUN
            currentTimeGet(&downsendtv[nTtlIndex]);
#endif
			CommBeginTime[nTtlIndex] = time(NULL);
...
#if STAT_FUN
                InfoMsgData[nTtlIndex].nPayloadLength = 1;
#else
...
#if STAT_FUN
                uint16 index = 1;
                struct timeval downrecvtv;
                currentTimeGet(&downrecvtv);
                memcpy_r(&InfoMsgData[nTtlIndex].aPayloadData[index],(uint8*)&downsendtv[nTtlIndex].tv_sec,4);
                index+=4;
                memcpy_r(&InfoMsgData[nTtlIndex].aPayloadData[index],(uint8*)&downsendtv[nTtlIndex].tv_usec,4);
                index+=4;
                memcpy_r(&InfoMsgData[nTtlIndex].aPayloadData[index],(uint8*)&downrecvtv.tv_sec,4);
                index+=4;
                memcpy_r(&InfoMsgData[nTtlIndex].aPayloadData[index],(uint8*)&downrecvtv.tv_usec,4);
                index+=4;
                InfoMsgData[nTtlIndex].nPayloadLength = index;
#endif
				PlcWriteQueue(pAmrSendUp, (uint8 *)&InfoMsgData[nTtlIndex], sizeof(MSG_INFO_T)+InfoMsgData[nTtlIndex].nPayloadLength);
				break;
...
#if STAT_FUN
		gGlobalSeqInfo[nTtlIndex].FrameSeqInfo[i].msgrecvtime = AmrModeActive.recvtm;
        currentTimeGet(&gGlobalSeqInfo[nTtlIndex].FrameSeqInfo[i].downsendtime);
#endif
...
#if STAT_FUN
    FRAME_SEQ_INFO_T *tmp = &gGlobalSeqInfo[nArrIndex].FrameSeqInfo[SeqIndex];
    
    writeMsgDealTime(tmp->MeterAddr,&tmp->msgrecvtime,&tmp->downsendtime,&tmp->downrecvtime,0);
#endif
        ...
        if (RecvLen > 0)
        {
...
#if STAT_FUN
            currentTimeGet(&gGlobalSeqInfo[nTtlIndex].FrameSeqInfo[SeqIndex].downrecvtime);
#endif
            switch(ret)
            {
            case CHECK_RESULT_DENY:
            ...
            case CHECK_RESULT_SUCCEED:
                if ((AppData.AFN == 0xF1) && (AppData.Fn == 1))
                {
                    ...
                    InfoMsgData.aPayloadData[IndexID++] = 0x00;  //PLC_READING_INFO 结束 保留 3字节
#if STAT_FUN
                    FRAME_SEQ_INFO_T *tmp = &gGlobalSeqInfo[nTtlIndex].FrameSeqInfo[SeqIndex];
                    memcpy_r(&InfoMsgData.aPayloadData[IndexID],(uint8*)&tmp->downsendtime.tv_sec,4);
                    IndexID+=4;
                    memcpy_r(&InfoMsgData.aPayloadData[IndexID],(uint8*)&tmp->downsendtime.tv_usec,4);
                    IndexID+=4;
                    memcpy_r(&InfoMsgData.aPayloadData[IndexID],(uint8*)&tmp->downrecvtime.tv_sec,4);
                    IndexID+=4;
                    memcpy_r(&InfoMsgData.aPayloadData[IndexID],(uint8*)&tmp->downrecvtime.tv_usec,4);
                    IndexID+=4;
#endif
                    ...
                    writeMsgDealTime(tmp->MeterAddr,&tmp->msgrecvtime,&tmp->downsendtime,&tmp->downrecvtime,1);
#endif
...
#if STAT_FUN
    currentTimeGet(&AmrModeActive.recvtm);
#endif
...
#if STAT_FUN
    currentTimeGet(&AmrSlaveNodeMonitor.recvtv);
#endif
...
int8 CreateTaskByInfo(void)
{
    ...
    switch (stInfoData.IOP_ID)
    {
        ...
        case 0x0081: /* 初始化 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 初始化\n");
            CcoInitMsg(&stInfoData, amr);
            break;
        }
        case 0x0082: /* 查询本地模块参数 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询本地模块参数\n");
            CcoQueryParaMsg(&stInfoData, amr);
            break;
        }
        case 0x0083: /* 查询模块地址 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询模块地址\n");
            CcoQueryAddrMsg(&stInfoData, amr);
            break;
        }
        case 0x0084: /* 查询节点信息 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询节点信息\n");
            CcoQueryNodeInfoMsg(&stInfoData, amr);
            break;
        }
        case 0x0085: /* 查询节点相位信息 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询节点相位信息\n");
            CcoQueryNodePhaseMsg(&stInfoData, amr);
            break;
        }
        case 0x0086: /* 查询模块串口参数 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询模块串口参数\n");
            CcoQueryUartParaMsg(&stInfoData, amr);
            break;
        }
        case 0x0087: /* 查询模块协议模式 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询模块协议模式\n");
            CcoQueryPrtlModeMsg(&stInfoData, amr);
            break;
        }
        case 0x0088: /* 模块地址设置 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 模块地址设置\n");
            CcoSetAddrMsg(&stInfoData, amr);
            break;
        }
        case 0x0089: /* 从节点上报设置 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 从节点上报设置\n");
            CcoSetStaReportMsg(&stInfoData, amr);
            break;
        }
        case 0x0090: /* 启动获取广播延时 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 启动获取广播延时\n");
            CcoStartGetBroadDelayMsg(&stInfoData, amr);
            break;
        }
        case 0x0091: /* 广播 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 广播\n");
            CcoBroadcastMsg(&stInfoData, amr);
            break;
        }
        case 0x0092: /* 启动从节点主动注注册 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 启动从节点主动注注册\n");
            CcoStartStaRegMsg(&stInfoData, amr);
            break;
        }
        case 0x0093: /* 结束当前任务 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 结束当前任务\n");
            CcoEndCurrentTaskMsg(&stInfoData, amr);
            break;
        }
        case 0x0094: /* 启动预告任务执行 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 启动预告任务执行\n");
            CcoStartPreviewExecMsg(&stInfoData, amr);
            break;
        }
        case 0x0095: /* 预告抄读对象 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 预告抄读对象\n");
            CcoPreviewMeterObjMsg(&stInfoData, amr);
            break;
        }
        case 0x0096: /* 设置通信协议模式 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 设置通信协议模式\n");
            CcoSetPrtlMsg(&stInfoData, amr);
            break;
        }
        case 0x0097: /* 设置模块通信速率 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 设置模块通信速率\n");
            CcoSetRateMsg(&stInfoData, amr);
            break;
        }
        case 0x0098: /* 设置恢复默认速率时长 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 设置恢复默认速率时长\n");
            CcoSetRestoreDurationMsg(&stInfoData, amr);
            break;
        }
        case 0x0099: /* 设置最高通信速率 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 设置最高通信速率\n");
            CcoSetMaxRateMsg(&stInfoData, amr);
            break;
        }
        case 0x0100: /* 数据转发（国网） */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 数据转发（国网）\n");
            CcoDataFwdMsg(&stInfoData, amr);
            break;
        }
        case 0x0101: /* 数据透传 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 数据透传\n");
            CcoDataTransMsg(&stInfoData, amr);
            break;
        }
        case 0x0102: /* 上报从节点信息 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报从节点信息\n");
            break;
        }
        case 0x0103: /* 上报事件数据 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报事件数据\n");
            break;
        }
        case 0x0104: /* 上报抄读内容请求 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报抄读内容请求\n");
            CcoReportMeterAck(&stInfoData, amr);
            break;
        }
        case 0x0105: /* 上报预告的通信对象数据 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报预告的通信对象数据\n");
            CcoReportPreviewObjAckMsg(&stInfoData, amr);
            break;
        }
        case 0x0106: /* 上报广播延时 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报广播延时\n");
            CcoReportBroadDelayAckMsg(&stInfoData, amr);
            break;
        }
        case 0x0107: /* 上报广播完成 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 上报广播完成\n");
            CcoReportBroadFinishAckMsg(&stInfoData, amr);
            break;
        }
        case 0x0108: /* 查询抄读方式 */
        {
            CCOUP_FMT_DEBUG("CreateTaskByInfo() 查询抄读方式\n");
            CcoQueryReadModeMsg(&stInfoData, amr);
            break;
        }
#endif
        default:
...
void AmrInfoInit(uint8 nArrIndex)
{
    ...
    //从本地私有文件读取协议模式
    Ret = read_pdata_xram(gModuleManuName[nArrIndex], (char *)&ccoMode, CCO_MODE_XRAM_OFFSET, sizeof(ccoMode));
    if(Ret < 0)
    {
        CCODOWN_FMT_DEBUG(nArrIndex, "AmrInfoInit() read_pdata_xram(gModuleManuName[%d] error! \n", nArrIndex);
    }
    ...
    {
        CCODOWN_FMT_DEBUG(nArrIndex, "AmrInfoInit() 本地私有文件获取 协议模式=%u\n", ccoMode);

        if (ccoMode == CCO_MODE_FUJIAN || ccoMode == CCO_MODE_GW)
        {
            g_CcoMode = ccoMode;
        }
    }
...
void PlcTaskCreat(uint8    nArrIndex)
{
    ...
    //厂商模块获取成功后再执行其他任务
    if ((g_CcoMode == CCO_MODE_GW) && (gModuleManuID[nTtlIndex] == MMID_NULL) && (gPlcTaskDetail[nTtlIndex].TaskName != MS_GET_MANU_ID))
    {
#ifdef AREA_FUJIAN
        CcoModeNegotiateProc(nTtlIndex);
#endif
    ...
    if (g_CcoMode == CCO_MODE_GW && gModuleManuID[nTtlIndex] == MMID_NULL)
    {
        return;
    }
    ...
    */
    ...
    //档案比对任务的判断
    if (g_CcoMode == CCO_MODE_GW && (1 == gMeterFileCompareFlag[nTtlIndex]) && (0 == gMeterFileModifyState[nTtlIndex]))
    {

...
extern EVENT_3000_INFO event_3000;   //失压事件
extern EVENT_3001_INFO event_3001;   //欠压事件
extern EVENT_3002_INFO event_3002;   //过压事件
extern EVENT_3003_INFO event_3003;   //断相事件
extern EVENT_3005_INFO event_3005;   //过流事件
extern EVENT_300F_INFO event_300F;   //电压逆相事件
extern EVENT_301D_INFO event_301D;   //电压不平衡事件
extern EVENT_301E_INFO event_301E;   //电流不平衡事件
...
//extern HUDP hUdp;                                             //数据中心通信句柄
//extern uint8               TnmEventReport;                      //允许终端主动上报标志,1允许,0不允许
extern YC_DATA_CONFIG      g_aYcDataConfig[MAX_YC_NUM];
extern YC_DATA_SEND        g_aYcDataSend[MAX_YC_NUM];  //遥测数据存储区
extern METER_FILE_T        g_sMeterFileAll;
...
/*-                             函数                                 -*/
/*-************************************** ** ************************-*/
void CheckERC_3000(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //失压
void CheckERC_3001(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //欠压
void CheckERC_3002(mtrnum_t mtr_id, OOP_METER_T *oopMeter);	//过压
void CheckERC_3003(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //断相
void CheckERC_3005(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //过流
void CheckERC_300F(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //逆相
void CheckERC_301D(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //电压不平衡
void CheckERC_301E(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //电流不平衡
void CheckERC_3106(void);                                   //终端停上电
void CheckERC_3119(mtrnum_t mtr_id, OOP_METER_T *oopMeter); //电流反极性
void CheckSelfERC(void);                                    //自检
...
//<End Common Problem>
void  InitPowerDown(void);		//-终端上电保电事件监测-
void  InitEvent(void);			//-事件监测初始化-
int   PowerDownCheck(void);		//停电时间检测
int   TmnPowerProc(void);
...
typedef struct //故障信息上报参数数据结构，上报按带时标的单点遥信上报
{
	...
	uint16      reportAddr[12]; //上报遥测信息体地址(取遥测点表信息体地址的存放位置)
} FAULT_REPORT_RAMINFO;

#if HEBEI_FUN == 1
#define PARA_ERC11_FILE		"^check_erc11_papa"
#endif
...
#if ANHUI_FUN ==1
BOOL GetRecentdata_3106(uint8 *buff,uint16* len);
uint8 PowerOffTimeCheck(int checkFlag, uint8 *tmnPowerOffEvent);

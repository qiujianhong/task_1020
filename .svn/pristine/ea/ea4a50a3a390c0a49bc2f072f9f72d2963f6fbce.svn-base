...
static int uart_recv_data(struct tag_UART_DEVICE *dev,        uint8 *buf, uint32 len)
{
    ...
    if(recvlen > 0 && 0 == strcmp(pDev->base.szDeviceID, UART_RS4851_DEVICE_ID_STR))
    {
        ...
        printf("[%04d-%02d-%02d %02d:%02d:%02d.%03ld]recvlen %d-%d:", stime->tm_year + 1900, stime->tm_mon + 1, stime->tm_mday, 
                stime->tm_hour, stime->tm_min, stime->tm_sec, tv.tv_usec/1000, recvlen, len);

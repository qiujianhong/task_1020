...
int systime_set_to_smiOS(DateTimeHex_t pDateTime)
{
    ...
    ret = gui_send_syncmsg(MSG_GUI_RTC, sendbuf, offset, recvbuf, 32, &recLen);
...
int IPconfig_set_to_smiOS(char* Name, int IP, int NetMask, int NetGw)
{
    char  sendbuf[100] = {0};
    char  recvbuf[32] = {0};
    uint16 recLen = 0;
	...
	int len = sizeof(Name);
	sendbuf[0] = len;
	memcpy(sendbuf + 1, Name, len);

	sendbuf[len + 1] = 4;
	len += 2;
	commfun_IPToHexStr(IP, &sendbuf[len]);
	len += 4;
	sendbuf[len] = 4;
	len += 1;
	commfun_IPToHexStr(NetMask, &sendbuf[len]);
	len += 4;
	sendbuf[len] = 4;
	len += 1;
	commfun_IPToHexStr(NetGw, &sendbuf[len]);
	len += 4;

	GUI_BUF_TRACE(sendbuf, len, "send buf:");

    ret = gui_send_syncmsg(MSG_GUI_SETIP, (uint8*)&sendbuf, len, (uint8*)&recvbuf, 32, &recLen);
    if ((ret != ERR_OK) || (recLen != 1) || (recvbuf[0] != 1))
    {
        GUI_FMT_DEBUG("Ipconfig_set_to_smiOS err! ret = %d, reclen = %d\n", ret, recLen);
        nRet = ERR_NORMAL;
    }
    else
    {
...
int Get_Eth_Status()
{
    ...
    int    ret = 0;
    int    nRet = 0;
	//char  *stringvalue = "eth0";
	uint8 playload[10];
	uint16 length=g_ETHName[0][0];

	playload[0]=length;
	memcpy(playload+1,(uint8*)&g_ETHName[0][1],length);
    ret = gui_send_smios(0, SYSTEM_IOP_GET_ETHSTATUS, playload, length+1);

    if ((ret != ERR_OK) )
    {
       GUI_FMT_DEBUG("SYSTEM_IOP_GET_ETHSTATUS err! ret = %d\n", ret);
        nRet = ERR_NORMAL;
    }
    else
    {

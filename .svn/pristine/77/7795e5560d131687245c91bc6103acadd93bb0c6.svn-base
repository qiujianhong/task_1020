...
OOP_WARNINGOUT_T g_AppWarnGroup;//0xf2060200 告警时段状态
...
* @Update    :
**********************************************************************/
int appctrl_write_param(int cid, uint32 uOAD, uint16 infonum, void *pData, uint32 len)
{
    NOMAL_OAD_T tOAD;
...
void appctrl_get_arg(int cid)
{
    ...
    if(0 != appctrl_get_oad(cid, 0xf2060200, 0, &g_AppWarnGroup, sizeof(g_AppWarnGroup)))
    {
...
/**********************************************************************
* @name      : appctrl_set_oopswitch
* @brief     ：设置数据中心继电器状态(3次防抖)
* @param[in] ：uint8 acm1         设备1
               uint8 acm2        设备2
               uint8 *input      接入状态(跳闸)
               uint8 *selfinput  接入状态(跳闸)
* @param[out]：
* @return    ：
* @Create    : 王津剑
* @Date      ：2022-7-1
...
{
    OOP_SWITCHOUT_T tSwitch;
    static uint8    input1[OOP_MAX_SWITCHOUT] = {0xFF};
    static uint8    input2[OOP_MAX_SWITCHOUT] = {0xFF};
    ...
    for(i = 0; i < OOP_MAX_SWITCHOUT / 2; i++)
    {
        ...
        if(input2[i] == input[i] && input1[i] == input[i])
        {
            tSwitch.value[i].input = input[i];
        }
        input2[i] = input1[i];
        input1[i] = input[i];
    }
    ...
    for(i = 0; i < OOP_MAX_SWITCHOUT / 2; i++)
    {
        if(input2[i + OOP_MAX_SWITCHOUT / 2] == selfinput[i] && input1[i + OOP_MAX_SWITCHOUT / 2] == selfinput[i])
        {
            tSwitch.value[i + OOP_MAX_SWITCHOUT / 2].input = selfinput[i];
        }
        input2[i + OOP_MAX_SWITCHOUT / 2] = input1[i + OOP_MAX_SWITCHOUT / 2];
        input1[i + OOP_MAX_SWITCHOUT / 2] = selfinput[i];
    }
...
{
    appctrl_write_param(g_CtrlDBid, 0x80020300, 0, (uint8*)pFee, sizeof(OOP_CALLFEE_T));
    appctrl_write_param(g_CtrlDBid, 0x80027fff, 0, &state, sizeof(uint8));
}
...
void appmain_write_warn_status(OOP_WARNINGOUT_T *groupstatus)
{
    ...
    if(0 == memcmp(&g_AppWarnGroup, groupstatus, sizeof(OOP_WARNINGOUT_T)))
    {
        return;
    }
    ...
    tOAD.classtag = CLASS_DATA_NORMAL;
    memcpy(&g_AppWarnGroup, groupstatus, sizeof(OOP_WARNINGOUT_T));
    db_write_nomal(g_CtrlDBid, &tOAD, (uint8*)&g_AppWarnGroup, sizeof(OOP_WARNINGOUT_T));
}

...
CAPP_ARRAY=()
MAPP_ARRAY=(smiOS dbCenter desktopGui acMeter rspSample usbManager scsMonitor uartManager wirelessDCM taskManager stAmr)
...
if [ -z "$APP_NAME" -o "all" == "$APP_NAME" ]; then
	...
	dateZ=`grep -a -o '[0-9][0-9][0-9][0-9][0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/$AREA_NAME/$AREA_NAME""_fun.c | sed -n 9p`
	if [ "$AREA_NAME" == "default" ]; then
		# 获取版本信息
		verZ=`grep -a -o 'O3Z[0-9]-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/default/default_fun.c | sed -n 1p`
	else
		# 获取版本信息
		verZ=`grep -a -o '3[A-Z]*[0-9]*-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/$AREA_NAME/$AREA_NAME""_fun.c | sed -n 1p`
	fi
	...
	tar -zcvf package_3358_ZCU3.$AREA_NAME.$verZ"_"$dateZ.tar.gz *
	mv package_3358_ZCU3.$AREA_NAME.$verZ"_"$dateZ.tar.gz $basePath/release/tmp_s/
	...
	# MD5值校验
	echo "package_3358_ZCU3.$AREA_NAME.$verZ""_""$dateZ.tar.gz" > $packagePath/md5.txt
	MD5=`md5sum $packagePath/package_3358_ZCU3.$AREA_NAME.$verZ"_"$dateZ.tar.gz | awk '{ print $1}'`

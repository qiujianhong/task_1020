...
uint32 g_CcoProduct = CCO_PRODUCT_MODULE;
    ...
    while(1)
    {
        ...
        switch (SocketState)
        {
            case IPC_SOCK_INIT:
            {
                ...
                if (g_CcoProduct == CCO_PRODUCT_MODULE && 0 == gMapManagerAppLoginFlag)//查看模组管理器是否注册成功
                {
                ...
                else if(g_CcoProduct == CCO_PRODUCT_MODULE && 0 == gDBCenterAppLoginFlag)//查看数据中心是否注册成功
                {
            ...
            case IPC_SOCK_RECV:
            {
                ...
                if(RecvLen > 0)
                {
                    ...
                    else if(g_CcoProduct == CCO_PRODUCT_MODULE && ptMsgInfo->IID == 0x0004)//模组管理器
                    {
                        switch (ptMsgInfo->IOP)
                        {
                            case 0x0001://模块插拔事件
                ...
                if (g_CcoProduct == CCO_PRODUCT_MODULE && (0 == gMapManagerAppLoginFlag || 0 == gDBCenterAppLoginFlag))
                {
...
* @Update    :
**********************************************************************/
int CcoUartSend(int fd, uint8 *data, int len)
{
    int send_len = 0;  
...
* @Update    :
**********************************************************************/
int CcoUartRecv(int fd, uint8* pBuffer, int nBufLen, int rsptime, int rcvtime)
{
        ...
        switch (SocketState)
        {
            case IPC_SOCK_INIT:                    //初始化串口
                ...
                if((NULL == gDevHalInfo[nTtlIndex].dev) || (1 == gReloadDevFlag[nTtlIndex]))
                {
                    ...
                    uint32 baud = 9600;
                    AppData_T appData = {0};
                    AppData_T recvAppData = {0};
                    uint8 sendBuf03F4[256] = {0};
                    uint16 sendLen03F4 = 0;
                    uint8 recvBuf03F4[512] = {0};
                    uint16 recvLen03F4 = 0;
                    uint32  retryCnt = 0;
                    uint32  speedCheckflag = 0;

                    // 打开设备
                    while (1)
                    {
                        ret = gDevHalInfo[nTtlIndex].dev->uart_param_set(gDevHalInfo[nTtlIndex].dev, baud, 8, 1, PARITY_EVEN);  //TRUE:1, FALSE:0
                        ...
                        if (ret == 0)
                        {
                            while (1)
                            {
                                sendLen03F4 = CcoPackAFN03F4(appData, sendBuf03F4, nTtlIndex);
                                CCODOWN_BUF_DEBUG(nTtlIndex, sendBuf03F4, sendLen03F4, "CcoPackAFN03F4\n");

                                if (CcoUartSend(gDevHalInfo[nTtlIndex].dev->fd, sendBuf03F4, sendLen03F4) == 0)
                                {
                                    CCODOWN_FMT_DEBUG(nTtlIndex, "CcoUartSend succ fd=%d\n", gDevHalInfo[nTtlIndex].dev->fd);
                                    recvLen03F4 = CcoUartRecv(gDevHalInfo[nTtlIndex].dev->fd, recvBuf03F4, sizeof(recvBuf03F4), 500, 200);

                                    if (recvLen03F4 > 0)
                                    {
                                        CCODOWN_BUF_DEBUG(nTtlIndex, recvBuf03F4, recvLen03F4, "CcoUartRecv succ\n");
                                        UnPackFrameData(recvLen03F4, recvBuf03F4, &recvAppData);

                                        if (recvAppData.AFN == 0x03 && recvAppData.Fn == 4)
                                        {
                                            CCODOWN_FMT_DEBUG(nTtlIndex, "speed Check succ\n");
                                            retryCnt = 0;
                                            speedCheckflag = 1;
                                            break;//接收成功
                                        }
                                        ...
                                        {
                                            CCODOWN_FMT_DEBUG(nTtlIndex, "speed Check fail AFN=0x%02X,Fn=%u\n", recvAppData.AFN, recvAppData.Fn);
                                            retryCnt++;
                                            sleep(2);
                                    ...
                                    {
                                        CCODOWN_FMT_DEBUG(nTtlIndex, "CcoUartRecv len=0\n");
                                        retryCnt++;
                                        sleep(2);
                                ...
                                {
                                    CCODOWN_FMT_DEBUG(nTtlIndex, "CcoUartSend fail fd=%d\n", gDevHalInfo[nTtlIndex].dev->fd);
                                    retryCnt++;
                                    sleep(2);
                            ...
                            if (speedCheckflag == 1)
                            {
                                speedCheckflag = 0;
                                SocketState = IPC_SOCK_RECV;
                                CCODOWN_FMT_DEBUG(nTtlIndex, "串口[%d]速率%u bps DownTtlHalThread 交互尝试成功\n", nTtlIndex, baud);
                                gReloadDevFlag[nTtlIndex] = 0;
                                break;
                            }
                            else
                            {
                                CCODOWN_FMT_DEBUG(nTtlIndex, "串口[%d]速率%u bps DownTtlHalThread 交互尝试失败\n", nTtlIndex, baud);
                        ...
                        {
                            CCODOWN_FMT_DEBUG(nTtlIndex, "串口[%d]打开失败DownTtlHalThread Error \n", nTtlIndex);
                            sleep(2);
                            break;
                        }
                        sleep(1);
                    ...
                    while (1)
                    {
                        ...
                        if (ret == 0)
                        {
                            ...
                            if (speedCheckflag == 1)
                            {
                                speedCheckflag = 0;
                                SocketState = IPC_SOCK_RECV;
                                CCODOWN_FMT_DEBUG(nTtlIndex, "串口[%d]速率%u bps DownCcuThread 交互尝试成功\n", nTtlIndex, baud);
                                gReloadDevFlag[nTtlIndex] = 0;
#ifdef AREA_FUJIAN
                                amr->flag.initFlag = UINT_MAX;
#endif
                            ...
                            {
                                CCODOWN_FMT_DEBUG(nTtlIndex, "串口[%d]速率%u bps DownCcuThread 交互尝试失败\n", nTtlIndex, baud);
...
void *AmrThread(void *arg)
{
	...
	if((nTtlIndex >= MAX_DOWN_THREAD_NUM)
		|| (nTtlIndex >= MAX_ALLOWED_DOWN_THREAD_NUM))
	{
		CCODOWN_FMT_DEBUG(nTtlIndex, "AmrThread() 线程数组下标nTtlIndex=%d异常! \n", nTtlIndex);
		return NULL;
	}
...
int init_config()
{
    ...
    if(NULL != ini)
    {
        ...
        printf("读取mqttport = %d \n", mqttport);
        flag = iniparser_getint(ini, "MODULE:FLAG", CCO_PRODUCT_MODULE);
        printf("MODULE:FLAG=%d\n", flag);
        g_CcoProduct = flag == CCO_PRODUCT_MODULE ? CCO_PRODUCT_MODULE : CCO_PRODUCT_NO_MODULE;
        printf("g_CcoProduct=%u\n", g_CcoProduct);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (g_CcoProduct == CCO_PRODUCT_MODULE)
    {
        //创建下行通信线程
    ...
    while(1)
    {
        ...
        if (g_CcoProduct == CCO_PRODUCT_MODULE && (1 == gMapManagerAppLoginFlag && 1 == gDBCenterAppLoginFlag && 1 == bAppSelfLoginState))
        {
            //查询模组信息

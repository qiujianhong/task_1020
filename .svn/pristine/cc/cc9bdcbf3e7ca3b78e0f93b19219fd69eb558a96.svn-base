...
* @return 成功返回ERR_OK；失败返回错误编码
*/
int32 power_battery_state_get(struct tag_POWER_DEVICE *dev, int32 *state)
{  
    int32 iRet = ERR_OK;
    //3个缓存, 同时为1则拔出
    static int32 s_Ret[3] = {0, 0, 0};

    int i = 0;
    while(i < 3)
    {
        iRet = get_gpio_value(SCM_PIN_PC21);
        //默认插入
        if(iRet<0)
        {
            iRet = 0;
    ...
    if(s_Ret[0] == 1 && s_Ret[1] == 1 && s_Ret[2] == 1)
    {
    	  *state = 1;
    }
    else 
...
int32 power_battery_out_set(struct tag_POWER_DEVICE *dev, uint8 type, uint8 val)
{
    ...
    iRet = set_gpio_value(val, SCM_PIN_PC17);

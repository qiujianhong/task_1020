...
void ClearStatData(uint8 msn)
{
	...
	sprintf(pfile,"rm -f /data/private/statpdata%u",msn);
...
{    
    ...
    int     ret = 0;
    uint8   EndSymbol = 0;
    if (g_guiMain.pluginCur->dwId == 0x00000025)
    {
       ret = read_pdata_xram(GUI_RELAY_PN_FILE, (char *)&EndSymbol, 0, 1); 
       if (ret < 0 || (ret == 0 && EndSymbol != 0x55))
       {
           ClosePopLayer();
           return;
       }

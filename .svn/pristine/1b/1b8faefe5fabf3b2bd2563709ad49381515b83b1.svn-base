...
static char *aCanDevice[CAN_DEV_NUM] = {CAN0_DEVICE_ID_STR, CAN1_DEVICE_ID_STR};
...
{
    int fd = -1;
    if(0 == strcmp(device_id, CAN0_DEVICE_ID_STR))
    {
        fd = open(CAN0_DEV_NAME, O_RDWR|O_NOCTTY|O_NONBLOCK);
    }
    else if(0 == strcmp(device_id, CAN1_DEVICE_ID_STR))
    {
        fd = open(CAN1_DEV_NAME, O_RDWR|O_NOCTTY|O_NONBLOCK);
    }
    else
    {
        return ERR_NOTEXIST;

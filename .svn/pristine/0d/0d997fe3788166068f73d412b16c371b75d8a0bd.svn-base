...
int32 PlcInitTTL(COMM_PARA_T *pCommPara)
{
    ...
    if (fdTTL == -1)                            
    {           
        printx("Can't Open Serial Port %s\n", DeviceNameTTL);
        return -1;      
    }   
    
    /*锁串口*/
    if (PlcSetlock(fdTTL, F_WRLCK) == -1)       
    {
        close(fdTTL);
        return -2;
    }
    ...
    /*设置串口波特率*/   
    if (PlcSetSpeed(fdTTL, BaudRateTTL) == -1)  
    {
        printx("Set speed Error %s\n", DeviceNameTTL);
        close(fdTTL);
        return -3;
    }   
    
    /*设置串口参数 数据位 校验 停止位*/                                           
    if (PlcSetParity(fdTTL, DataBitsTTL, StopBitsTTL, ParityFlagTTL) == 0) 
    {
        printx("Set Parity Error %s   DataBitsTTL %d, StopBitsTTL %d, ParityFlagTTL %d\n",
                DeviceNameTTL, DataBitsTTL, StopBitsTTL, ParityFlagTTL);
        close(fdTTL);
        return -4;
    }
...
int32 PlcRecvTTL(int fdTTL, int32 speed, uint8 *pBuffer, int32 nBufLen)
{
    ...
    /*调用read()，进行串口数据接收*/
    recv_len = read(fdTTL, pBuffer, nBufLen); 
...
int32 PlcSendTTL(int32 fdTTL, uint8* pBuffer, int32 nBufLen)
{
    ...
    /*发送数据*/
    send_len = write(fdTTL, pBuffer, nBufLen); 

...
#define LOCAL_IP_INFO_FE0	"/etc/network/interfaces.d/FE0"
...
{
	g_valid_pwd.nIndex = 1;
	//g_guiMain.bValidatePwd = TRUE;
...
void ValidatePwdDraw(HDC hdc)
{
	...
	int nLeft=32;
	int nTop=74;
	TextOut(hdc,nLeft,nTop, g_valid_pwd.czPwd);
	
	nLeft = 110;
	if((uint32)g_valid_pwd.nIndex == 0)
	{
		ITEM_SELECT;
	...
	// 绘制OK部分
	nTop += 28;
	if((uint32)g_valid_pwd.nIndex == 1)
	{
		ITEM_SELECT;
	...
	if((uint32)g_valid_pwd.nIndex == 2)
	{
		ITEM_SELECT;
...
void ValidatePwdReturn(char* pNewStr, void* pPara)
{
	if(strlen(pNewStr) != PASSWORD_LEN && memcmp("334455", pNewStr, 6) != 0)
	{
		...
		{
			InputBox(g_valid_pwd.czPwd, ValidatePwdReturn, NULL, 0x01 | 0x04 | 0x08 | 0x10, 0x08);
		}
	}
	else
	{
		memcpy(g_valid_pwd.czPwd, pNewStr, strlen(pNewStr)+1);
		IsValidatePwd();
	}
}
...
BOOL ValidatePwdKeyMsg(int nKey)
{
	...
	case KEY_OK:
		{// 确认处理
			...
			else if((uint32)g_valid_pwd.nIndex == 1)
			{// 处理确定按键
				return IsValidatePwd();
			}
			else if((uint32)g_valid_pwd.nIndex == 2)
			{// 处理取消键
				ClosePopLayer();
			}
		}
...
void ValidatePwdInit(DWORD dwPluginId)
{
	...
	//g_guiComm.bValidatePwd = TRUE;
	memset(g_valid_pwd.czPwd, '0',PASSWORD_LEN);
	/*#if defined AREA_FUJIAN || defined AREA_JIANGXI
		memcpy(g_valid_pwd.czPwd, "000000", sizeof(g_valid_pwd.czPwd));
	#else
		memcpy(g_valid_pwd.czPwd, PASSWORD_STR, sizeof(g_valid_pwd.czPwd));
	#endif*/

...
void appctrl_elec_init()
{
    ...
    ret = appctrl_read_ctrl(CTRL_ELEC_TYPE, (uint8*)buf, 4096, &len);
...
//    offset += sprintf(pBuf + offset, "\n定值      : %lld.%04lldkWh\n", 
//                      g_EcBuf.MondefValue[group]/10000, g_EcBuf.MondefValue[group]%10000);
//    offset += sprintf(pBuf + offset, "报警系数  : %d%%\r\n", g_EcBuf.MonlimitCoe[group]);
//    offset += sprintf(pBuf + offset, "浮动系数  : %d%%\n", g_EcBuf.MonfloatCoe[group]);
//    offset += sprintf(pBuf + offset, "当月总电量: %lld.%04lldkWh", 
//                      g_EcBuf.MonUsedValue[group]/10000, g_EcBuf.MonUsedValue[group]%10000);
...
void appctrl_elec_write(uint8 bDay)
{
    ...
    ret = appctrl_write_ctrl(CTRL_ELEC_TYPE, (uint8 *)&g_EcBuf, sizeof(EC_INFO_T));
    if(0 != ret)
    {

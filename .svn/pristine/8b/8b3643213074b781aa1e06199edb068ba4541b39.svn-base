...
sint32 parser_msg(uint8 *rxbuf, sint32 rxbuf_len, MSG_OBJ_T *msgObj)
{
    ...
    dolog_debug("msgObj->prm = %d", msgObj->prm);
    dolog_debug("msgObj->priority = %d", msgObj->priority);
    dolog_debug("msgObj->index = %d", msgObj->index);
    dolog_debug("msgObj->label = %d", msgObj->label);
    dolog_debug("msgObj->source = %s", msgObj->source);
    dolog_debug("msgObj->destination = %s", msgObj->destination);
    dolog_debug("msgObj->prm = 0x%x", msgObj->tag);
    dolog_debug("msgObj->length = %d", msgObj->length);
#endif
    return ret;
}
...
sint32 pro_parser_msg(uint8 *rxbuf, uint32 rxbuf_len, MSG_OBJ_T *msgObj)
{
    ...
    do
    {
        ...
        {
            dolog_error("msg source len[%d] > MAX_SOURCE_LENGTH", sourceLen);
            break;
        }

        if ((uint8 *)(msgObj->source) - rxbuf + sourceLen >= rxbuf_len)
        {
            dolog_error("msg source too long");
            break;
        }
        ...
        {
            dolog_error("msg destinationLen len[%d] > MAX_DESTINATION_LENGTH", destinationLen);
            break;
        }

        if ((uint8 *)(msgObj->destination) - rxbuf + destinationLen >= rxbuf_len)
        {
            dolog_error("msg destination too long");
            break;
        }
        ...
        {
            dolog_error("msg source and destination is identical");
            break;
        }
...
//        msgObj->length = GET_LENGTH(len_addr);
        ...
        // if (msgLen > 1000)
        // {
        //     dolog_error("msg err 2 msgLen:[%u] rxbuf_len:[%u]", msgLen,
        //                 rxbuf_len);
        //     hdolog_debug(rxbuf, rxbuf_len);
        // }

        if (msgLen > rxbuf_len)
        {
            dolog_error("msg err 2 msgLen:[%u] rxbuf_len:[%u]", msgLen, rxbuf_len);
            hdolog_debug(rxbuf, rxbuf_len);
            break;
        }
...
// #define MSG_HEADER_DEBUG
        ...
        dolog_debug("msgObj->source = %s", msgObj->source);
        dolog_debug("msgObj->destination = %s", msgObj->destination);
        dolog_debug("msgObj->tag = 0x%x", msgObj->tag);
        dolog_debug("msgObj->length = %d", msgObj->length);
#endif

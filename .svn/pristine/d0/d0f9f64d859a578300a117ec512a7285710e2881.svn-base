...
int32 WriteLicFile(char *devinfofile, const char *license, int32 license_size)
{
    int32 fd = 0;
    int32 iRet= 0;
    struct flock lock;

    if(NULL == devinfofile || NULL == license)
    {
        return ERR_PTR;
    }
...
int32 devinfo_write_applicense(struct tag_DEVINFO_DEVICE *dev, const char *appno, const char *license, int32 license_size)
{
    ...
    sprintf(devinfofile, "%s%s.lic", APP_ID_STORE_PATH, noBig);
    iRet = WriteLicFile(devinfofile, license,  license_size);
        ...
        else if(DT_REG == ptr->d_type) //普通文件
        {
            if(NULL != strstr(ptr->d_name, ".lic"))
            {
                ...
                if(count == index + 1)
                {
                    ...
                    ret = devinfo_read_applicense(dev, appno, license, max_license_size);
                    break;
                }
            }
        }

...
# 需要手动添加编译内容及其路径
LIB_ARRAY=(appgui msgassist msgbus security storage framecomm algo iniparser mapmsg mxml mqttclient template cJson mingather ble radio uart sm3sum)
...
APP_ARRAY=(desktopGui taskManager stAmr acMeter rspSample lcMonitor pdsManager btManager usbManager scsMonitor uartManager mapManager wirelessDCM smiOS dbCenter ccoRouter)
APP_DIR=($CODE_DIR/app_base_desktopGui/code  $CODE_DIR/app_edg_taskManager/code $CODE_DIR/app_adv_stAmr/code \
         $CODE_DIR/app_base_acMeter/code $CODE_DIR/app_base_rspSample/code $CODE_DIR/app_edg_lcMonitor/code $CODE_DIR/app_edg_pdsManager/code \
		 $CODE_DIR/app_base_btManager/code $CODE_DIR/app_base_usbManager/code $CODE_DIR/app_sys_secManager/code \
		 $CODE_DIR/app_base_uartManager/code $CODE_DIR/app_base_mapManager/code $CODE_DIR/app_base_wirelessDCM/code $CODE_DIR/app_sys_smiOS/code \
		 $CODE_DIR/app_base_dbCenter/code $CODE_DIR/app_base_ccoRouter/code) 

if [ "801" == "$BOARD_NAME" ]; then
	APP_ARRAY+=(iotManager)
	APP_DIR+=($CODE_DIR/app_base_iotManager_Res/code)
else
	APP_ARRAY+=(usbManager iotManager cpuMonitor)
	APP_DIR+=($CODE_DIR/app_base_usbManager/code $CODE_DIR/app_base_iotManager/code $CODE_DIR/app_sys_cpuMonitor/code)
fi
...
#入参判断执行
if [ "801" != "$BOARD_NAME" -a "3358" != "$BOARD_NAME" ]; then
	# 核心板判断
			...
			if [ "$PROG_NAME" == "${APP_ARRAY[$i]}" ]; then
				NAME_FLAG=1
                 if [ -z "$CLEAN_FLAG" ];then
                    #编译单个app
                    makeapp $i
				   ./package_$BOARD_NAME".sh" $AREA_NAME $PROG_NAME

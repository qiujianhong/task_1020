...
#define HIGH_Z_CUR                    0x00200     //零序电流超上限
#define OVER_LOAD                     0x00400     //负荷过载
#define CUR_NBLC                      0x00800     //电流不平衡
#define VLT_NBLC                      0x01000     //电压不平衡
#define VLT_HARA                      0x02000     //A相电压谐波总畸变率超限
#define VLT_HARB                      0x04000     //B相电压谐波总畸变率超限
#define VLT_HARC                      0x08000     //C相电压谐波总畸变率超限
#define CUR_HARA                      0x20000     //A相电流谐波总畸变率超限
#define CUR_HARB                      0x40000     //B相电流谐波总畸变率超限
#define CUR_HARC                      0x80000     //C相电流谐波总畸变率超限
#define SHORT_CIRCUIT                 0x100000    //电流短路
#define REPORWER_OVER                 0x200000    //无功过补偿
#define REPORWER_UNDER                0x400000    //无功欠补偿
...
#define     RTUSelfCheckAddr1    (Tmn_Rtc_Info_ID+1)    // 1字节，配电变压器停上电事件记录，D0表示终端停电，D4表示是否变化，其他备用
#define     RTUSelfCheckAddr     (RTUSelfCheckAddr1+9)    // 2字节，终端事件记录，D0表示终端停电，D1表示自检异常，其他备用
#define     GBLostvltAddr        (RTUSelfCheckAddr+9)   // 9字节，电压断相记录地址
#define     GBLowvltAddr         (GBLostvltAddr+9)      // 9字节，电压失压记录地址
#define     GBVltUpAddr          (GBLowvltAddr+9)       // 9字节，电压越上限记录地址
#define     GBVltDownAddr        (GBVltUpAddr+9)        // 9字节，电压越下限记录地址
...
#define     GBEnUpAddr           (GBCurrAbnormalAddr+9) // 9字节，负荷过载记录地址
#define     GBNoEnUpAddr         (GBEnUpAddr+9)         // 9字节，无功过补偿记录地址
#define     GBNoEnLowAddr        (GBNoEnUpAddr+9)       // 9字节，无功欠补偿记录地址
#define     GBZeroCurrUpAddr     (GBNoEnLowAddr+9)      // 9字节，零序电流偏大记录地址
#define     GBVltDisUpAddr       (GBZeroCurrUpAddr+9)   // 9字节，电压畸变率越限记录地址
#define     GBCurrDisUpAddr      (GBVltDisUpAddr+9)     // 9字节，电流畸变率越限记录地址
...
void  CurReverseEvent(int clientid, uint8 bType);             //电流反极性检测
void  VltBreakEvent(int clientid, uint8 bType);               //电压失压、断相判断
void PhaseReverseEvent(int clientid, uint8 bType);                        //逆相序判断
void  PhaseReverseEvent07(mtrnum_t mtr_id, uint8 type);                                 //逆相序判断
void  VltCurImbalanceEvent(int clientid, uint8 bType);        //电压不平衡事件发生/恢复
void  VltCurImbalanceEvent07(mtrnum_t mtr_id, uint8 type);                              //电压不平衡事件发生/恢复
void  VltSupassEvent(int clientid, uint8 bType);               //电压越限及恢复
void  VltSupassEvent07(mtrnum_t mtr_id, uint8 type);                                    //电压越限及恢复
void  CurHighEvent(int clientid, uint8 bType);                //电流越限及恢复

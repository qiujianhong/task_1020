...
BOOL AddMenus(LPXNode pnodeMenus, int nParentId)
{
	...
	for(int i = 0; i < nCnt; i ++)
	{
		...
		{// 没有PluginId,表示为最终菜单项，
            ...
            printf("%s:添加最终菜单项：%s nParentId%d\n",__FUNCTION__,pName, nParentId);
            AddMenu(nParentId, (char*)pName, FALSE, 0, 0, 0, 0, 0, 0, bVisible,
		...
		else
		{
            ...
            printf("%s:添加非最终菜单项：%s nParentId %d ID %d\n",__FUNCTION__,pName, nParentId, Id);
            // 进入递归
...
BOOL ReadXML()
{
    ...
    while(xmlPath[0] == 0)
    {
    	sleep(1);		//等待xmlPath初始化
    }
	FILE* pFile = fopen(xmlPath, "r");
	if(pFile == NULL)
	{
        ASSERT(FALSE);
		return FALSE;

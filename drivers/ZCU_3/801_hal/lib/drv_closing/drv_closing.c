...
* @Update    :
**********************************************************************/
int closing_IIC_Prepare(int fd, uint8 yd_out)
{
    if(fd <= 0)
    {
        return -1;
...
* @Update    :
**********************************************************************/
int closing_IIC_Contactor(int fd, uint8 ctrl_out)
{
    if(fd <= 0)
    {
        return -1;
...
void *closing_switch_later(void *pArg)
{
    ...
    while(1)
    {
        ...
        memcpy(&tNode, &(pList->queue[pList->head]), sizeof(CTRL_SWITCH_NODE_T));
...
int32 closing_turn(struct tag_CLOSING_DEVICE *dev, uint8 id, CLOSING_TURN_E turn)
{
    ...
    if(CLOSING_TURN_ON == turn)
    {
        switch(id)
        {
            case 1: status = 0x08; break;
            case 2: status = 0x04; break;
            case 3: status = 0x02; break;
            case 4: status = 0x01; break;
            default: break;
        }
    }
...
static int drv_closing_open(HW_MODULE *module, const char* device_id, HW_DEVICE **device)
{
    ...
    closing_IIC_Prepare(dev->fd, PREPARE_OFF); //送检打开预动

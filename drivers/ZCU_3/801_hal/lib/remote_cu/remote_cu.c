...
int32 remote_cu_on_level_set(struct tag_REMOTE_CU_DEVICE *dev, uint8 val)
{
    ...
    ret = ioctl(fd, 1 - val, REMOTE_CU_ONOFF_ARG);
    close(fd);
...
int32 remote_cu_insert_state_get(struct tag_REMOTE_CU_DEVICE *dev, int32 *state)
{
    ...
    *state = 0;
    ...
    while(bReadCount++ < 10)
    {
        ...
        data1 = ((data1 >> 16) & 0x00000001);
        usleep(1000);
        
        read(fd, &data2, sizeof(data2));
        data2 = ((data2 >> 16) & 0x00000001);
        usleep(1000);
        
        read(fd, &data3, sizeof(data3));
        data3 = ((data3 >> 16) & 0x00000001);
        usleep(1000);
        
        read(fd, &data4, sizeof(data4));
        data4 = ((data4 >> 16) & 0x00000001);
        usleep(1000);
        
        read(fd, &data5, sizeof(data5));
        data5 = ((data5 >> 16) & 0x00000001);
        //连续5次读到相同的值，防止干扰

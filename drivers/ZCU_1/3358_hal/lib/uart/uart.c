...
#define   UART_DEV_NAME_USB4       "/dev/ttyUSB4"


#define   UART_SYS_DEV0_PATH       "/sys/bus/usb/devices/2-1.3:1.0/tty"
#define   UART_SYS_DEV1_PATH       "/sys/bus/usb/devices/2-1.3:1.2/tty"
#define   UART_SYS_DEV2_PATH       "/sys/bus/usb/devices/2-1.3:1.4/tty"
#define   UART_SYS_DEV3_PATH       "/sys/bus/usb/devices/2-1.3:1.6/tty"

#define   UART_DEV_NAME_LCM        "/dev/ttyLCM" 
#define   UART_DEV_NAME_LJM        "/dev/ttyLJM" 
...
static int uart_id_to_num(const char *id)
{
    ...
    if(strcmp(id, UART_RADIO_DEVICE_ID_STR) == 0)
    {
        return TTYS0;
    }
    ...
    else if(strcmp(id, UART_DEVICE_LCM_STR) == 0)
    {
        return TTYS_LCM;
    }
    else if(strcmp(id, UART_DEVICE_LJM_STR) == 0)
    {
        return TTYS_LJM;
    }
...
int uart_id_to_usbdev(const char *id, char *dev, int devlen)
{
    ...
    else if(0 == strcmp(id, UART_USB_DIAG_DEVICE_ID_STR))
    {
        ...
        else if(0 == access(UART_DEV_NAME_USB2, F_OK))
        {
            memcpy(dev, UART_DEV_NAME_USB2, strlen(UART_DEV_NAME_USB2));
            return strlen(UART_DEV_NAME_USB2);
        }
...
static int uart_id_to_devpath(const char *id, char *dev, int devlen)
{
    ...
    if(strcmp(id, UART_RADIO_DEVICE_ID_STR) == 0)
    {
        memcpy(dev, UART_DEV_NAME_TTYS0, strlen(UART_DEV_NAME_TTYS0));
        return strlen(UART_DEV_NAME_TTYS0);
    }
    else if(strcmp(id, UART_REMOTE_CU_DEVICE_ID_STR) == 0)
    {
    ...
    {
        memcpy(dev, UART_DEV_NAME_LCM, strlen(UART_DEV_NAME_LCM));
        return strlen(UART_DEV_NAME_LCM);
    }
    else if(strcmp(id, UART_DEVICE_LJM_STR) == 0)
    {
        memcpy(dev, UART_DEV_NAME_LJM, strlen(UART_DEV_NAME_LJM));
        return strlen(UART_DEV_NAME_LJM);
    }

...
#define   GPIO_BACKLIGHT	AT91_PIN_PC17   /*lcd背光管脚 */
...
int32 lcd_active_id_set(struct tag_LCD_DEVICE *dev, uint32 active_id)
{
   ...
   {
   		printf("Get id Return value of:%u\n",local_id);
        return ERR_PTR;
   }
...
int32 lcd_bkl_set(struct tag_LCD_DEVICE *dev, LCD_BKL_TYPE_E eBkl)
{
    ...
    if(eBkl == LCD_BKL_OFF)
        ret = set_gpio_value(GPIO_BACKLIGHT, GPIO_OUT_LOW);
    else if(eBkl == LCD_BKL_ON)
        ret = set_gpio_value(GPIO_BACKLIGHT, GPIO_OUT_HIGH);
	
    if(ret < 0)
    {
        printf("gpio set error ret%d eBkl[%d], GPIO_BACKLIGHT[%x], GPIO_OUT_LOW[%x], GPIO_OUT_HIGH[%x].\n", ret, eBkl, GPIO_BACKLIGHT, GPIO_OUT_LOW, GPIO_OUT_HIGH);
        ret = ERR_NORMAL;
    }
    return ret;
}
    ...
    if((colstart%8) == 0)
    {
       // printf("colstart%8 == 0\n");
    	for(iRow=0; iRow <iRectHight; iRow++)
    	{
    	    ...
    	    ptr = ((uint8 *)( pfb_buffer)) + 30*rowstart + colstart/8 + iRow*30;

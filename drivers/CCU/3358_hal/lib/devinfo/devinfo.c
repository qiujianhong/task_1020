...
#define MTD_UBOOT_DEV   "/tmp/uboot.img"
#define MTD_KERNEL_DEV   "/tmp/boot.img"
                              ...
                              int32 max_license_size)
{
    ...
    else
    {
        ...
        lseek(fd, APPLIC_LEN * 2, SEEK_SET);
                               ...
                               const char *license, int32 license_size)
{
    ...
    sprintf(devinfofile, "%s%s.lic", APP_ID_STORE_PATH, noBig);
    iRet = WriteLicFile(devinfofile, appname, licBig, license_size);
    sprintf(devinfofile, "%s%s.lic", APP_ID_BACKUP_PATH, noBig);
    WriteLicFile(devinfofile, appname, licBig, license_size);

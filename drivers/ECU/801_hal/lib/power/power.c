...
static int usb_gpio[] = {
        -1,
        GPIO_34, /* usb module 1 5V_EN1 */
        GPIO_37, /* usb module 2 5V_EN2 */
        GPIO_38, /* usb module 3 5V_EN3 */
        GPIO_63, /* usb module 4 5V_EN1 */
        GPIO_55, /* usb module 5 5V_EN1 */
        SCM_PIN_PB3, /* Bluetooth rst pin */
        GPIO_41, /* usb hub reset */
};
#endif

//充电
#define CAP_CHARGE_PIN     10
#define BAT_CHARGE_PIN     SCM_PIN_PD21

//放电
#define CAP_DISCHARGE_PIN  11
#define BAT_DISCHARGE_PIN  21
...
int32 power_battery_charge_set(struct tag_POWER_DEVICE *dev, uint8 type, uint8 val)
{
    ...
    if(NULL == dev || type > 1 || val > 1)
    {
        return -1;
    }

    gpiovaule = 0 == val ? GPIO_OUT_LOW : GPIO_OUT_HIGH;
    iRet = set_gpio_value(0 == type ? BAT_CHARGE_PIN : CAP_CHARGE_PIN, gpiovaule);
    return iRet;
}
...
int32 power_battery_out_set(struct tag_POWER_DEVICE *dev, uint8 type, uint8 val)
{
    ...
    gpiovaule = 0 == val ? GPIO_OUT_LOW : GPIO_OUT_HIGH;
    iRet = set_gpio_value(0 == type ? BAT_DISCHARGE_PIN : CAP_DISCHARGE_PIN, gpiovaule);
    return iRet;
}
...
* @brief 获取电池拔插状态
* @param[in] dev: 设备描述
* @param[out] state: 电池拔插，0插入，1 拔出
* @return 成功返回ERR_OK；失败返回错误编码
*/
int power_battery_state_get(struct tag_POWER_DEVICE *dev, int32 *state)
{
    uint8  type = 1;
    uint32 data1 = 1;
    uint32 data2 = 2;
         ...
         HW_DEVICE **device)
{
    ...
    dev->power_ply_vcc_state_get = power_ply_vcc_state_get;
#endif
    dev->power_battery_state_get = power_battery_state_get;

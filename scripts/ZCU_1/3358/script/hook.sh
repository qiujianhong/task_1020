...
if [ ! $PID ]; then
	/usr/sbin/mosquitto -c /etc/mosquitto/mosquitto.conf >/dev/null 2>&1 &
...
APPLIST=(smiOS dbCenter desktopGui acMeter rspSample usbManager radioManager scsMonitor uartManager wirelessDCM taskManager stAmr)

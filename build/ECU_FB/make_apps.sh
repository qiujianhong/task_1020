...
# 需要手动添加编译内容及其路径
LIB_ARRAY=(appgui msgassist security storage framecomm iniparser mapmsg mqttclient template uart cJson)
LIB_DIR=($LIB_DIR/appgui $LIB_DIR/msgassist $LIB_DIR/security $LIB_DIR/storage $LIB_DIR/commonfun $LIB_DIR/iniparser $LIB_DIR/mapmsg $LIB_DIR/mqttclient $LIB_DIR/template $LIB_DIR/uart $LIB_DIR/cJson )
...
#入参判断执行
if [ "701" != "$BOARD_NAME" -a "3358" != "$BOARD_NAME" ]; then
	# 核心板判断

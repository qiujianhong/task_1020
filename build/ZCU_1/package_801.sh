...
CAPP_ARRAY=()
MAPP_ARRAY=(smiOS dbCenter desktopGui acMeter rspSample scsMonitor uartManager wirelessDCM taskManager stAmr radioManager)
...
if [ -z "$APP_NAME" -o "all" == "$APP_NAME" ]; then
	...
	if [ "$AREA_NAME" == "default" ]; then
		# 获取版本信息
		verZ=`grep -a -o 'O1Z[0-9]-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/default/default_fun.c | sed -n 2p`
	else
		# 获取版本信息
	...
	#801的专1，显示不太一样(阴阳码，镜像显示)，lcdctrl需要修改，这里直接拷贝修改好的lcdctrl
	mv lcdctrl $basePath/release/tmp_s/lcdctrl
	tar -zcvf package_801_ZCU1.$AREA_NAME.$verZ"_"$dateZ.tar.gz *
	mv package_801_ZCU1.$AREA_NAME.$verZ"_"$dateZ.tar.gz $basePath/release/tmp_s/
	...
	# MD5值校验
	echo "package_801_ZCU1.$AREA_NAME.$verZ""_""$dateZ.tar.gz" > $packagePath/md5.txt
	MD5=`md5sum $packagePath/package_801_ZCU1.$AREA_NAME.$verZ"_"$dateZ.tar.gz | awk '{ print $1}'`

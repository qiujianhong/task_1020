...
OSPath=$basePath/depends/OS/ECU/3358
IAMGEPath=$basePath/depends/image
upgradePath=$basePath/release/upgrade

CAPP_ARRAY=(taskManager stAmr dbCenter ccoRouter lcMonitor)
...
if [ -z "$APP_NAME" -o "all" == "$APP_NAME" ]; then
	...
	if [ "$AREA_NAME" == "default" ]; then
		# 获取版本信息
		verZ=`grep -a -o 'O1E[0-9]-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/default/default_fun.c | sed -n 1p`
	else
		# 获取版本信息
	...
	tar -zcvf package_3358_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz *
	mv package_3358_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz $basePath/release/tmp_s/
	...
	# MD5值校验
	echo "package_3358_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz" > $packagePath/md5.txt
	MD5=`md5sum $packagePath/package_3358_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz | awk '{ print $1}'`

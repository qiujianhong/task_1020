...
otherapppath=$basePath/scripts/ECU/area_apps
OSPath=$basePath/depends/OS/ECU/801
IAMGEPath=$basePath/depends/image
upgradePath=$basePath/release/upgrade

CAPP_ARRAY=(taskManager stAmr lcMonitor)
MAPP_ARRAY=(desktopGui acMeter rspSample btManager iotManager)
 MAPP_ARRAY=(smiOS wirelessDCM desktopGui acMeter mapManager rspSample btManager usbManager scsMonitor iotManager uartManager)
...
if [ -z "$APP_NAME" -o "all" == "$APP_NAME" ]; then
	...
	dateZ=`grep -a -o '[0-9][0-9][0-9][0-9][0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/$AREA_NAME/$AREA_NAME""_fun.c | sed -n 13p`
	if [ "$AREA_NAME" == "default" ]; then
		# 获取版本信息
		verZ=`grep -a -o 'O1E[0-9]-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/default/default_fun.c | sed -n 2p`
	else
		# 获取版本信息
		verZ=`grep -a -o 'E[A-Z]*[0-9]*-[0-9][0-9]' $basePath/apps/app_adv_puAmr/code/area/$AREA_NAME/$AREA_NAME""_fun.c | sed -n 1p`
	fi
	echo "Area: "$AREA_NAME
	...
	tar -zcvf package_801_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz *
	mv package_801_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz $basePath/release/tmp_s/
	...
	# MD5值校验
	echo "package_801_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz" > $packagePath/md5.txt
	MD5=`md5sum $packagePath/package_801_ECU.$AREA_NAME.$verZ"_"$dateZ.tar.gz | awk '{ print $1}'`

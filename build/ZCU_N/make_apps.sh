...
# 需要手动添加编译内容及其路径
LIB_ARRAY=(msgassist security msgbus storage framecomm algo iniparser mapmsg mqttclient template cJson mingather uart ble radio)
LIB_DIR=($LIB_DIR/msgassist $LIB_DIR/security $LIB_DIR/msgbus $LIB_DIR/storage $LIB_DIR/commonfun $LIB_DIR/algorithm $LIB_DIR/iniparser $LIB_DIR/mapmsg \
         $LIB_DIR/mqttclient $LIB_DIR/template $LIB_DIR/cJson $LIB_DIR/mingather $LIB_DIR/uart $LIB_DIR/ble $LIB_DIR/radio)

# yc和stamr是一套代码, 需要后编译yc
APP_ARRAY=(smiOS dbCenter desktopGui wirelessDCM acMeter rspSample taskManager lcMonitor stAmr uartManager btManager usbManager)
APP_DIR=($CODE_DIR/app_sys_smiOS/code $CODE_DIR/app_base_dbCenter/code $CODE_DIR/app_base_desktopGuiQT/desktopGui $CODE_DIR/app_base_wirelessDCM/code \
         $CODE_DIR/app_base_acMeter/code $CODE_DIR/app_base_rspSample/code $CODE_DIR/app_edg_taskManager/code $CODE_DIR/app_edg_lcMonitor/code \
		 $CODE_DIR/app_adv_stAmr/code $CODE_DIR/app_base_uartManager/code $CODE_DIR/app_base_btManager/code $CODE_DIR/app_base_usbManager/code)		 
...
makeapp(){
	...
	if [ -e "${APP_DIR[$1]}" ]; then
		...
		cd $CURR_DIR
		mkdir -p ${APP_ARRAY[$1]} && cd ${APP_ARRAY[$1]}
		if [ "desktopGui" == "${APP_ARRAY[$1]}" ]; then
			cmake -DQT_BORAD=1 ${APP_DIR[$1]} && make -j4 
		else
			cmake ${APP_DIR[$1]} && make -j4
		fi
	fi
    echo -e "\033[32m=============================end=============================\033[0m"	
	cd $CURR_DIR
...
# 依赖库解压
if [ ! -d /opt/qt5.12 ]; then
	cd $CURR_DIR/../../depends/qt/3358
	tar -zxvf qt5.12.tar.gz --directory=/opt
fi
if [ ! -d /opt/tslib ]; then
	cd $CURR_DIR/../../depends/tslib/3358
	tar -zxvf tslib.tar.gz --directory=/opt
fi
	
#入参判断执行
if [ "3358" != "$BOARD_NAME" ]; then
	# 核心板判断
